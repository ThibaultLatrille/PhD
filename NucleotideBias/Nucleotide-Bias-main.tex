...
\begin{figure}[htbp]
    ...
    In the right panel, the mean scaled fixation probability of non-synonymous mutations is restricted to mutations from weak nucleotides (AT) to strong nucleotides (GC), called $\avgpfix_{AT \rightarrow GC}$, is represented in the vertical axis.
...
From an alignment of protein-coding \acrshort{DNA} sequences, without knowing the specific history of substitutions, models of inference can estimate the mutational bias ($\lambda$) and mean scaled fixation probability ($\avgpfix$).
...
The likelihood of the data can be computed from the tree topology and the instantaneous rates between codons, known as the codon substitution matrix ($\Submatrix$).
Here we developped two models of inference, differing only by their parametrization of the matrix $\Submatrix$, considered homogeneous along the sequence (not site-specific).
...
\subsubsection{$\omega$ as a scalar with Muse \& Gaut formalism}
At the level of nucleotides, the mutation rate is defined entirely by a generalized time-reversible nucleotide mutation rate matrix $\Mutmatrix$, as a function of the nucleotide frequencies $\Mutequi$ and the symmetric exchangeability rates $\Exchan$~\citep{Tavare1986}:
\begin{equation}
...
Altogether, the $61$-by-$61$ codon substitution matrix of \citet{Muse1994} is defined by the mutation matrix $\Mutmatrix$, a single parameter of selective strength $\omega$, and the genetic code:
\begin{equation}
    \begin{dcases}
...
From the maximum likelihood estimates of the \acrshort{GTR} mutation matrix ($\widehat{\Mutmatrix}$), we can estimate of the mutational bias toward $\mathrm{AT}$ as $\widehat{\lambda}_{\text{MG}} = (\widehat{\mutequi_A} +\widehat{\mutequi_T}) /(\widehat{\mutequi_G} +\widehat{\mutequi_C}) $.
...
\subsubsection{$\omega$ as a tensor with mean-field derivation}
...
We can also estimate the mean scaled fixation probability of non-synonymous mutations $\widehat{\omega}_{\text{MF}}$ (see section~\ref{sec-mut-bias:mean-field-omega}) which is a compound parameter of the mutation matrix and the mean scaled fixation probability in the different direction.
...
\begin{figure}[htbp]
    ...
    Estimated mutational bias ($\widehat{\lambda}$) in the vertical axis as a function of mutational bias ($\lambda$) in the horizontal axis, for two different models of inference.
    In the left panel, modelling a single mean scaled fixation probability as in Muse \& Gaut formalism leads to skewed estimation of the mutation bias.
    Modelling multiple mean scaled fixation probability leads to precise estimation of the mutation bias}
...
\begin{equation}
    D(\profile\siteexp) = \exp \left[ - \sum\limits_{\aSetAa} \profile\siteexp_{\aminoacid} \ln \left( \profile\siteexp_{\aminoacid} \right) \right]
\end{equation}
...
\begin{equation}
     D \left(  \left\langle \profile \right\rangle \right) = \exp \left[ - \sum\limits_{\aSetAa} \left\langle \profile \right\rangle_{\aminoacid} \ln \left( \left\langle \profile \right\rangle_{\aminoacid} \right) \right]
\end{equation}

\subsection{Mean scaled fixation probability}
\label{subsec:fixation-bias}
The mean scaled fixation probability of non-synonymous mutations is the ratio :
\begin{align}
...
\begin{align}
    \omega_{\text{MF}} & = \dfrac{ \sum\limits_{i} \sigma_{\ci[1]}\sigma_{\ci[2]}\sigma_{\ci[3]} \epsilon_{\aaj} \sum\limits_{j \in \NonSyn_{\ci}} \mutmatrix_{\nucitoj} \epsilon_{\aaj} \beta_{\aai, \aaj} }{\sum\limits_{i} \pi_{i} \sum\limits_{j \in \NonSyn_{\ci} } \mutmatrix_{\nucitoj}}
\end{align}

The mean scaled fixation probability from weak (AT) to strong (GC) nucleotides is $\omega_{\textbf{AT} \rightarrow \textbf{GC}}$ is obtained similarly by restricting the sum to one nucleotide mutation only from weak to strong.
