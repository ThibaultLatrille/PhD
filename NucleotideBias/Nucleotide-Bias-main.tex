...
This mean-field (\acrshort{MF}) model is fitted by maximum likelihood, giving an estimate for its parameters, $\widehat{\Mutmatrix}$, $\widehat{\AAExchan}$ and $\widehat{\AAEqui}$.
...
The mean scaled fixation probability of non-synonymous mutations $\widehat{\avgpfix}_{\text{\acrshort{MF}}}$ can also be computed.
...
The two alternative models of inference just considered, namely the classical Muse \& Gaut (\acrshort{MG}) and the mean-field (\acrshort{MF}) codon models, were then applied to empirical protein-coding sequence alignments.
...
Concerning selection, the estimated mean scaled fixation probability of non-synonymous mutations, denoted $\widehat{\avgpfix}$, is similarly estimated in the \acrshort{MG} and \acrshort{MF} models ($\widehat{\avgpfix}_{\text{\acrshort{MG}}} \simeq \widehat{\avgpfix}_{\text{\acrshort{MF}}}$).
...
\begin{table}[htbp]
    ...
    For the \acrshort{MG} model the mean scaled fixation probability of non-synonymous mutations ($\widehat{\avgpfix}_{MF}$) can be obtained either from weak (AT) to strong nucleotides (GC), or vice versa.
...
In addition, several other codon models~\citep{Rodrigue2008a,KosakovskyPond2020} should be included in a broader comparison of the accuracy of the estimation of the underlying mutational bias and strength of selection on protein-coding \acrshort{DNA} sequences.
