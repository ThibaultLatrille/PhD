...
To take into account the heterogeneity of selection between different sites of the protein, we assume that each site $\site$ of the sequence is independently evolving under a site-specific fitness landscape, characterized by a 20-dimensional frequency vector of scaled fitness parameters ($\Fit\siteexp$).
The fitness vectors across sites are drawn IID from a uniform Dirichlet distribution prior to the simulation over the tree:
\begin{equation}
    \Fit\siteexp \sim \text{Dirichlet} (\alpha, \hdots, \alpha), \Setsite
\end{equation}
...
For a site $\site$, the diversity $D\siteexp$ (or effective number of amino acids) is computed as the exponential of Shannon's entropy from the frequencies of the different amino-acids ($\profile\siteexp$):
\begin{equation}
...
\end{equation}
The diversity is a measure of the sparsity of the fitness profile, with a value of $1$ corresponding to a single peak (i.e.~with only one amino acid permissible), and a value of $20$ corresponding to a neutral landscape, where each amino acid has the same frequency.
