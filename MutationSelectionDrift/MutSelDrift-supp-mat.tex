...
In this model, each codon site $\site$ has its own fitness profile, denoted $\Profile\siteexp = \{ \Profile\siteexp_{\aaSource}, \forall 1 \leq \aaSource \leq 20 \}$, a vector of 20 amino-acid (Wrightian) fitnesses.
...
Altogether, for each possible mutant, we compute the selection coefficient of the mutant as:
\begin{equation}
    s \left( \Seqi^{t},\Seqi^{t+1}\right) = \sum\limits_{1 \leq \site \leq \Nsite} \ln \left( \dfrac{\Profile\siteexp_{\aaMap \left( \Seqi^{t+1}[ \site ] \right)}}{\Profile\siteexp_{\aaMap \left( \Seqi[ \site ] \right)}} \right),
\end{equation}
where $\Profile_{\site}$ used in this study are extracted from \citet{Bloom2017}, empirically determined in the context of deep mutational scanning.
