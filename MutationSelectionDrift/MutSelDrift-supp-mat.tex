...
For a category $\cat$, the Shannon entropy ($\entropy$) of the fitness profile ($\Profile$) is defined as:
\begin{equation}
...
\subsection{Site-specific fitness profiles (SimuDiv)}
...
\subsection{Wright-Fisher with polymorphism (SimuPoly)}
...
\subsection{Fisher geometric landscape (SimuGeo)}
...
\subsection{Protein folding probability (SimuFold)}
...
From a \acrshort{DNA} sequence $\Seqi^t$ after $t$ substitutions, we compute the free energy of the folded state $\Profile_{\mathrm{F}}\left(\Seqi^{t}\right)$, using the $3$-dimensional structure of the folded state and pair-wise contact energies between neighboring amino-acid residues:
\begin{equation}
...
\begin{equation}
    \GUnfold \left(\Seqi^{t}\right) = \left\langle G\left(\Seqi^{t}\right) \right\rangle - kT \ln (1.0\mathrm{E}^{160}) - \dfrac{2 \left[ \left\langle G\left(\Seqi^{t}\right)^2 \right\rangle - \left\langle G\left(\Seqi^{t}\right) \right\rangle^2\right] }{kT}
\end{equation}
