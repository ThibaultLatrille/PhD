...
\end{equation}
The Shannon's entropy measures the flatness of the fitness profile, with a value of $0$ corresponding to a single peak fitness landscape (only one amino acid is present), and a value of $log(20)\simeq3$ corresponding to a \gls{neutral} landscape, where each amino acid has the same fitness.
...
In this model, each \gls{codon} site $\site$ has its own fitness profile, denoted $\Profile\siteexp = \{ \profile\siteexp_{\aminoacid},\ 1 \leq \aminoacid \leq 20 \}$, a vector of 20 amino-acid scaled (Wrightian) fitness coefficients.
Since $\Seqi[ \site ]$ is the \gls{codon} at site $\site$, the encoded amino acid is $\aaMap \left( \Seqi[ \site ] \right)$, hence the fitness at site $\site$ is $\Profile\siteexp_{\aaMap \left( \Seqi[ \site ] \right)}$.
...
The next change in the protein coding \acrshort{DNA} and the time to next the event is chosen using Gillespie's algorithm~\citep{Gillespie1977}, according to the rates of \gls{substitution} between \glspl{codon}:
\begin{equation}
{\submatrix_{\itoj}}
    = \mu_{\itoj} \dfrac{4 \Ne s \left( \Seqi^{t},\Seqi^{t+1}\right)}{{1 - \e^{-4 \Ne s \left( \Seqi^{t},\Seqi^{t+1}\right)} }},
\end{equation}
where ${\submatrix_{\itoj}} = \mu_{\itoj}$ in the case of \gls{synonymous} \glspl{substitution}.
...
The population is assumed to be panmictic, with \gls{effective population size} $\Ne$ and with non-overlapping generations.
...
We simulated \glspl{substitution} in a protein using an adaptation of Fisher's geometric landscape~\citep{Tenaillon2014,Blanquart2016}.
...
From a \acrshort{DNA} sequence $\Seqi^t$ after $t$ \glspl{substitution}, the protein's \gls{phenotype} is given by:
\begin{equation}
...
We simulated \glspl{substitution} in the protein phosphatase ($\Nsite=300$ \gls{codon} sites) as in \citet{Goldstein2017}.
From a \acrshort{DNA} sequence $\Seqi^t$ after $t$ \glspl{substitution}, we compute the free energy of the folded state $ \GFold \left(\Seqi^{t}\right)$, using the 3-dimensional structure of the folded state and pair-wise contact energies between neighboring amino-acid residues:
\begin{equation}
...
$4$ independent inferences were performed on a randomly chosen set of $18$ coding sequences (\acrshort{CDS}) out of $226$.
Obtained with the mechanistic inference model developed in this paper of site-specific amino-acid fitness profiles and log-Brownian process for $\Ne$, $\mu$ and life-history traits.
Each plot is a correlation between a pair of experiments for a given parameter.
For each node (or branch) of the tree, the mean \gls{posterior} of the parameter over the \acrshort{MCMC} (after burn-in) is represented in blue dots, green solid lines are the $90\%$ confidence interval of the \acrshort{MCMC}.
...
\begin{table}[H]
    ...
    \begin{tabu}{|c|c|c|}
        ...
        \textbf{Experiment} & $\left\langle \entropy \right\rangle$ (\textbf{branch} $\Ne$) & $\left\langle \entropy \right\rangle$ (\textbf{constant} $\Ne$) \\ \hline
        \hline
        Mammals 18 \acrshort{CDS}, replicate~1, Chain 1 & $1.07 \pm 0.10$ & $1.14 \pm 0.10$\\ \hline
        Mammals 18 \acrshort{CDS}, replicate 2, Chain 2 & $1.07 \pm 0.09$ & $1.14 \pm 0.10$\\ \hline
        Mammals 18 \acrshort{CDS}, replicate 2, Chain 1 & $1.06 \pm 0.10$ & $1.12 \pm 0.09$\\ \hline
        Mammals 18 \acrshort{CDS}, replicate 2, Chain 2 & $1.06 \pm 0.09$ & $1.11 \pm 0.10$\\ \hline
        Mammals 18 \acrshort{CDS}, replicate 3, Chain 1 & $1.08 \pm 0.12$ & $1.15 \pm 0.11$\\ \hline
        Mammals 18 \acrshort{CDS}, replicate 3, Chain 2 & $1.04 \pm 0.10$ & $1.18 \pm 0.11$\\ \hline
        Mammals 18 \acrshort{CDS}, replicate 4, Chain 1 & $0.94 \pm 0.11$ & $1.02 \pm 0.12$\\ \hline
        Mammals 18 \acrshort{CDS}, replicate 4, Chain 2 & $0.89 \pm 0.11$ & $1.02 \pm 0.11$\\ \hline
        \hline
        Mammals 36 \acrshort{CDS}, replicate~1, Chain 1 & $1.02 \pm 0.06$ & $1.07 \pm 0.10$\\ \hline
        Mammals 36 \acrshort{CDS}, replicate~1, Chain 2 & $0.91 \pm 0.07$ & $1.03 \pm 0.07$\\ \hline
        Mammals 36 \acrshort{CDS}, replicate 2, Chain 1 & $0.92 \pm 0.09$ & $0.96 \pm 0.09$\\ \hline
        Mammals 36 \acrshort{CDS}, replicate 2, Chain 2 & $1.01 \pm 0.09$ & $1.02 \pm 0.11$\\ \hline
        Mammals 36 \acrshort{CDS}, replicate 3, Chain 1 & $0.93 \pm 0.00$ & $1.05 \pm 0.09$\\ \hline
        Mammals 36 \acrshort{CDS}, replicate 3, Chain 2 & $1.02 \pm 0.07$ & $1.05 \pm 0.11$\\ \hline
        Mammals 36 \acrshort{CDS}, replicate 4, Chain 1 & $1.04 \pm 0.07$ & $1.10 \pm 0.08$\\ \hline
        Mammals 36 \acrshort{CDS}, replicate 4, Chain 2 & $1.03 \pm 0.10$ & $1.08 \pm 0.08$\\ \hline
...
Obtained with the phenomenological inference model of log-Brownian process for the $\mu$ and the relative \gls{non-synonymous} \gls{substitution} rate ($\omega$), as in \citet{Lartillot2011}.
...
\begin{table}[H]
    ...
    Correlation coefficient between \gls{non-synonymous} \gls{substitution} rate~($\omega$), mutation rate per site per unit of time~($\mu$), and life-history traits (maximum longevity, adult weight and female maturity) were computed in placental mammals.
    Asterisks indicate strength of support ($\smash{^{*}} pp > 0.95$, $\smash{^{**}} pp > 0.975$).}
\end{table}
...
\begin{table}[H]
    ...
    Partial correlation coefficient between \gls{non-synonymous} \gls{substitution} rate~($\omega$), mutation rate per site per unit of time~($\mu$), and life-history traits (maximum longevity, adult weight and female maturity) were computed in placental mammals.
    Asterisks indicate strength of support ($\smash{^{*}} pp > 0.95$, $\smash{^{**}} pp > 0.975$).}
\end{table}
...
\begin{table}[H]
    ...
    Correlation coefficient between \gls{non-synonymous} \gls{substitution} rate~($\omega$), mutation rate per site per unit of time~($\mu$), and life-history traits (maximum longevity, adult weight and female maturity) were computed in primates.
    Asterisks indicate strength of support ($\smash{^{*}} pp > 0.95$, $\smash{^{**}} pp > 0.975$).}
\end{table}
...
\begin{table}[H]
    ...
    Partial correlation coefficient between \gls{non-synonymous} \gls{substitution} rate~($\omega$), mutation rate per site per unit of time~($\mu$), and life-history traits (maximum longevity, adult weight and female maturity) were computed in primates.
    Asterisks indicate strength of support ($\smash{^{*}} pp > 0.95$, $\smash{^{**}} pp > 0.975$).}
\end{table}
...
\begin{table}[H]
    ...
    \caption[Correlation coefficient matrix in Drosophila ($\Ne$)]{
...
\begin{table}[htbp]
    \renewcommand{\arraystretch}{0.5}
    \scriptsize \centering
    \input{drosophila/18CDS_SiteMutSelBranchNe_Rep_LogPopulationSize.tex}
...
\begin{table}[htbp]
    \renewcommand{\arraystretch}{0.5}
    \scriptsize \centering
    \input{drosophila/18CDS_SiteMutSelBranchNe_Rep_LogMutationRatePerTime.tex}
...
\begin{table}[H]
    ...
    Correlation coefficient between \gls{non-synonymous} \gls{substitution} rate~($\omega$), mutation rate per site per unit of time~($\mu$), and life-history traits (maximum longevity, adult weight and female maturity) were computed in Drosophila.
    Asterisks indicate strength of support ($\smash{^{*}} pp > 0.95$, $\smash{^{**}} pp > 0.975$).}
\end{table}
...
\begin{table}[H]
    ...
    Partial correlation coefficient between \gls{non-synonymous} \gls{substitution} rate~($\omega$), mutation rate per site per unit of time~($\mu$), and life-history traits (maximum longevity, adult weight and female maturity) were computed in Drosophila.
    Asterisks indicate strength of support ($\smash{^{*}} pp > 0.95$, $\smash{^{**}} pp > 0.975$).}
\end{table}
...
Then the probability of the \gls{substitution} history given the branch lengths ($\branchlength\branchexp = \mu\branchexp \branchtime\branchexp$), takes a very simple form:
\begin{equation}
...
\end{equation}
By Bayes theorem, the \gls{posterior} on $\CovarianceMatrix$, conditional on a particular realization of $\brownian$ (and thus of $\contrast$) is an invert Wishart distribution, of parameter $\covariancekappa \Identitymatrix + \Scattermatrix$ and with $\Nbranch + 3$ degrees of freedom.
\begin{equation}
