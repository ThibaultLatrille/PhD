...
%DIF DEL main-biorxiv.tex    Mon Apr 26 21:28:47 2021
%DIF ADD main-revision.tex   Thu May 13 14:29:34 2021
...
\usepackage{pgf,tikz}
%DIF 34c34-37
%DIF < \usetikzlibrary{positioning,arrows,automata,calc}
%DIF -------
\usetikzlibrary{shapes,arrows,backgrounds,fit,positioning,arrows,automata,calc} %DIF > 
\tikzset{res/.style={ellipse,draw,minimum height=1.0cm,minimum width=0.8cm}} %DIF > 
\tikzset{literal/.style={rectangle,draw,minimum height=0.5cm,minimum width=0.8cm,text width = 1.2 cm, align = center}} %DIF > 
 %DIF > 
%DIF -------
\usepackage{bm}
...
%DIF < 
%DIF < % Uncomment to override  the `A preprint' in the header
%DIF < %\renewcommand{\headeright}{}
...
% keywords can be removed
    ...
    Codon models \DIFaddbegin \DIFadd{allowing for variation in $\dnds$ across branches~\mbox{%DIFAUXCMD
\citep{Yang1998, Yang1998b, Yang2007, Dutheil2012} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend have been used to empirically measure such changes in the efficacy of purifying selection along phylogenies\DIFdelbegin \DIFdel{, either by allowing for different $\dnds$ values in different parts of the tree~\mbox{%DIFAUXCMD
\citep{Dutheil2012}}\hspace{0pt}%DIFAUXCMD
, or by estimating $\dnds$ independently for every branch of the tree~\mbox{%DIFAUXCMD
\citep{Popadin2007}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend .
    ...
    \DIFdelbegin \DIFdel{As an alternative to classical }\DIFdelend \DIFaddbegin \DIFadd{A first attempt in this direction was proposed by \mbox{%DIFAUXCMD
\citet{Nielsen2003}}\hspace{0pt}%DIFAUXCMD
, using a population-genetic argument to relate the distribution of }\DIFaddend $\dnds$ \DIFdelbegin \DIFdel{-based }%DIFDELCMD < {%%%
\DIFdel{codon}%DIFDELCMD < } %%%
\DIFdel{models, mechanistic }%DIFDELCMD < {%%%
\DIFdel{codon}%DIFDELCMD < } %%%
\DIFdel{models explicitly introduce population genetic equations into the }%DIFDELCMD < {%%%
\DIFdel{codon}%DIFDELCMD < } {%%%
\DIFdel{substitution}%DIFDELCMD < } %%%
\DIFdel{process~\mbox{%DIFAUXCMD
\citep{Halpern1998}}\hspace{0pt}%DIFAUXCMD
.
    Specifically, these so-called }\DIFdelend \DIFaddbegin \DIFadd{across sites with the underlying distribution of fitness effects.
    This first approach assumes that all non-synonymous mutations at a given site have the same selection coefficient.
    As a result of this assumption, there is a simple, one-to-one mapping between the $\dnds$ at a given site and the selection coefficient associated with all non-synonymous mutations at that site.
    In practice, different non-synonymous mutations are likely to have different fitness effects.
    In this direction, an alternative }\DIFaddend mutation-selection \DIFdelbegin %DIFDELCMD < {%%%
\DIFdel{codon }%DIFDELCMD < } %%%
\DIFdel{models explicitly assign }\DIFdelend \DIFaddbegin \DIFadd{codon modeling approach originally proposed by \mbox{%DIFAUXCMD
\citet{Halpern1998} }\hspace{0pt}%DIFAUXCMD
explicitly assigns }\DIFaddend a fitness parameter to each amino acid.
    ...
    \DIFdelbegin \DIFdel{Doing this would then }\DIFdelend \DIFaddbegin \DIFadd{This idea was previously explored in the context of two mechanistic models, relying on the distribution of $\dnds$ across sites~\mbox{%DIFAUXCMD
\citep{Nielsen2003} }\hspace{0pt}%DIFAUXCMD
or accounting for selection on codon usage~\mbox{%DIFAUXCMD
\citep{Nielsen2007}}\hspace{0pt}%DIFAUXCMD
.
    Doing this in the context of mutation-selection models with site-specific amino-acid preferences would provide }\DIFaddend provide an occasion to address several important questions: do we have enough signal in empirical sequence alignments, to estimate the evolutionary history of $\Ne$ along a phylogeny?
    ...
    In the model introduced here, $\Ne$ and $\mu$ are allowed to vary between species (\DIFdelbegin \DIFdel{across }\DIFdelend \DIFaddbegin \DIFadd{among }\DIFaddend branches) as a multivariate \DIFdelbegin \DIFdel{log-Brownian }\DIFdelend \DIFaddbegin \DIFadd{geometric Brownian }\DIFaddend process, but are assumed constant along the {DNA} sequence.
    ...
    \DIFaddbegin \DIFadd{Of note, since $\Ne$ and $\Fit$ are confounded parameters (equation~\ref{eq:mutationSelection}), the effective population size at the root is set to $1$ for identifiability of the fitness profiles.
    As a result, all values of $\Ne$ along the phylogeny are relative to that of the root, with a value of $\Ne > 1$ reflecting an increase in $\Ne$ along the branches (respectively a decrease for $\Ne < 1$) compared to the $\Ne$ at the root.
}\DIFaddend 
    ...
    \begin{figure*}[t]
        ...
        \end{minipage}
        \DIFdelbeginFL %DIFDELCMD < \llap{\raisebox{-1.0cm}{\scriptsize A\hspace{0.35cm}}}%%%
\DIFdelendFL \DIFaddbeginFL \llap{\raisebox{1.5cm}{\scriptsize \textbf{A}\hspace{4.2cm}}}\DIFaddendFL \hfill
        \begin{minipage}{0.32\linewidth}
            \includegraphics[width=\linewidth, page=1]{simulations/SimuPoly_SiteMutSelBranchNe_BranchCorrelation_Log10BranchLength}
        \end{minipage}
        \DIFdelbeginFL %DIFDELCMD < \llap{\raisebox{-1.0cm}{\scriptsize B\hspace{0.35cm}}}%%%
\DIFdelendFL \DIFaddbeginFL \llap{\raisebox{1.5cm}{\scriptsize \textbf{B}\hspace{4.2cm}}}\DIFaddendFL \hfill
        \begin{minipage}{0.32\linewidth}
            \includegraphics[width=\linewidth, page=1]{simulations/SimuFold_SiteMutSelBranchNe_BranchCorrelation_Log10BranchLength}
        \end{minipage}
        \DIFdelbeginFL %DIFDELCMD < \llap{\raisebox{-1.0cm}{\scriptsize C\hspace{0.35cm}}}%%%
\DIFdelendFL \DIFaddbeginFL \llap{\raisebox{1.5cm}{\scriptsize \textbf{C}\hspace{4.2cm}}}\DIFaddendFL \hfill
        \begin{minipage}{0.32\linewidth}
        ...
        \end{minipage}
        \DIFdelbeginFL %DIFDELCMD < \llap{\raisebox{-1.0cm}{\scriptsize D\hspace{0.35cm}}}%%%
\DIFdelendFL \DIFaddbeginFL \llap{\raisebox{1.5cm}{\scriptsize \textbf{D}\hspace{4.2cm}}}\DIFaddendFL \hfill
        \begin{minipage}{0.32\linewidth}
            \includegraphics[width=\linewidth, page=1]{simulations/SimuPoly_SiteMutSelBranchNe_BranchCorrelation_LogPopulationSize}
        \end{minipage}
        \DIFdelbeginFL %DIFDELCMD < \llap{\raisebox{-1.0cm}{\scriptsize E\hspace{0.35cm}}}%%%
\DIFdelendFL \DIFaddbeginFL \llap{\raisebox{1.5cm}{\scriptsize \textbf{E}\hspace{4.2cm}}}\DIFaddendFL \hfill
        \begin{minipage}{0.32\linewidth}
            \includegraphics[width=\linewidth, page=1]{simulations/SimuFold_SiteMutSelBranchNe_BranchCorrelation_LogPopulationSize}
        \end{minipage}
        \DIFdelbeginFL %DIFDELCMD < \llap{\raisebox{-1.0cm}{\scriptsize F\hspace{0.35cm}}}%%%
\DIFdelendFL \DIFaddbeginFL \llap{\raisebox{1.5cm}{\scriptsize \textbf{F}\hspace{3.9cm}}}\DIFaddendFL \hfill
        \caption[Inferred and simulated branch length and $\Ne$]{
            ...
            From left to right: simulation under the mutation-selection approximation (A,D), under a Wright-Fisher model accounting for small population size effects (5000 individuals at the root), site linkage and short term fluctuation of $\Ne$ (B,E)\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL and \DIFaddbeginFL \DIFaddFL{finally }\DIFaddendFL accounting for site epistasis in the context of selection for protein stability \DIFaddbeginFL \DIFaddFL{(C,F)}\DIFaddendFL .
            The tree root is $150$ million years old, where the initial population \DIFdelbeginFL \DIFdelFL{start }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{starts }\DIFaddendFL with a mutation rate of \DIFdelbeginFL \DIFdelFL{$\smash{1e^{-8}}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\smash{10^{-8}}$ }\DIFaddendFL per site per generation \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL and generation time of $10$ years.
    ...
    \begin{figure*}[t]
        ...
        \caption[Example of inferred $\Ne$ and $\mu$ on placental mammals dataset]{
            Inferred phylogenetic history of $\Ne$ (left) and $\mu$ (right) across placental mammals \DIFdelbeginFL \DIFdelFL{.
        Inference was conducted }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(posterior mean estimate), based }\DIFaddendFL on \DIFaddbeginFL \DIFaddFL{an analysis of }\DIFaddendFL a \DIFdelbeginFL \DIFdelFL{randomly chosen set }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{concatenation }\DIFaddendFL of 18 \DIFdelbeginFL \DIFdelFL{out of 226 highly conserved CDS ($< 1\%$ of gaps).
        Only highly conserved CDS were retained such that the assumption of constant fitness landscape is not incautiously broken by protein with changing function and/or adaptive }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{genes randomly chosen among single-copy orthologs putatively under an exclusively purifying }\DIFaddendFL selection \DIFaddbeginFL \DIFaddFL{regime}\DIFaddendFL .
            $\Ne$ \DIFdelbeginFL \DIFdelFL{values }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{estimates }\DIFaddendFL are relative to the \DIFdelbeginFL \DIFdelFL{root, which is arbitrarily set to one.
        Mean values }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{value }\DIFaddendFL of \DIFdelbeginFL %DIFDELCMD < {%%%
\DIFdelFL{MCMC}%DIFDELCMD < } %%%
\DIFdelFL{(after burn-in) are obtained }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\Ne$ }\DIFaddendFL at \DIFdelbeginFL \DIFdelFL{each node of }\DIFdelendFL the \DIFdelbeginFL \DIFdelFL{tree, hence a gradient can be extrapolated along each branch}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{root}\DIFaddendFL .
            \DIFaddbeginFL \DIFaddFL{The scale for }\DIFaddendFL $\mu$ \DIFdelbeginFL \DIFdelFL{spanned almost $2$ order of magnitude, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{is per nucleotide site }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{if }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{per total tree depth (i.e. total time from the most recent common ancestor to the present).
            If }\DIFaddendFL we assume the root to be $105$My old~\DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
\citep{Kumar2017}}\hspace{0pt}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\citep{Meredith2011,Kumar2017}}\hspace{0pt}%DIFAUXCMD
}\DIFaddendFL , the rescaled mutation rate per site per year in extant species is between \DIFdelbeginFL \DIFdelFL{$\smash{1.1e^{-10}}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\smash{1.1 \times 10^{-10}}$ }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{$\smash{7.8e^{-9}}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\smash{7.8 \times 10^{-9}}$}\DIFaddendFL .
            \DIFdelbeginFL \DIFdelFL{$\Ne$ at the root of the tree is arbitrarily set to $1$, and all values }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Icons }\DIFaddendFL are \DIFdelbeginFL \DIFdelFL{relative to the root, which spans at most an order of magnitude}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{adapted from }\url{http://phylopic.org}\DIFaddendFL .
        }
        \label{fig:mammals_popsize_and_mutrate}
    \end{figure*}

    We next applied our inference framework to a series of 4 empirical datasets spanning different \DIFdelbegin \DIFdel{taxnonomic }\DIFdelend \DIFaddbegin \DIFadd{taxonomic }\DIFaddend groups within metazoans.
    ...
    For computational reasons, we restricted our analyses to \DIFdelbegin \DIFdel{small concatenates made of }\DIFdelend \DIFaddbegin \DIFadd{a random set }\DIFaddend 18 \DIFdelbegin \DIFdel{randomly sampled alignments }\DIFdelend of orthologous genes\DIFdelbegin \DIFdel{.
Since }\DIFdelend \DIFaddbegin \DIFadd{, which are then concatenated into a single multiple sequence alignment (MSA) for analysis.
    Of note, }\DIFaddend the mutation-selection model considered here assumes \DIFdelbegin \DIFdel{a mostly nearly-neutral regime}\DIFdelend \DIFaddbegin \DIFadd{that the fitness profiles do not change with time.
    In contrast, some genes might experience fluctuating fitness landscapes through time.
    Such fluctuations are in fact one main cause of ongoing adaptation~\mbox{%DIFAUXCMD
\citep{Mustonen2009, Rodrigue2016}}\hspace{0pt}%DIFAUXCMD
.
    For that reason}\DIFaddend , genes for which positive selection was detected using a site codon model were excluded \DIFaddbegin \DIFadd{from the analysis}\DIFaddend .
    To assess the reproducibility of our inference and check that the signal about variation in $\Ne$ is not driven by particular genes, we analysed \DIFaddbegin \DIFadd{in total }\DIFaddend 4 \DIFdelbegin \DIFdel{concatenated random samples of }\DIFdelend \DIFaddbegin \DIFadd{different concatenated MSA each containing }\DIFaddend 18 \DIFaddbegin \DIFadd{randomly sampled }\DIFaddend genes.
    The different \DIFdelbegin \DIFdel{concatenate }\DIFdelend \DIFaddbegin \DIFadd{concatenated MSA }\DIFaddend showed similar trends in the change of $\mu$ \DIFdelbegin \DIFdel{($r^2=[0.92,0.95]$) }\DIFdelend and $\Ne$ \DIFdelbegin \DIFdel{($r^2=[0.51,0.68]$) }\DIFdelend between pairs of \DIFdelbegin \DIFdel{experiments }\DIFdelend \DIFaddbegin \DIFadd{replicates }\DIFaddend (see supplementary).
    ...
    The estimated covariance matrix (table~\ref{tab:mammals_correlation}) gives a global synthetic picture \DIFdelbegin \DIFdel{about }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend the patterns of covariation between the mutation rate per unit of time $\mu$, the {effective population size} $\Ne$ and the three {LHT}s.
    First, \DIFdelbegin \DIFdel{the variation in }\DIFdelend $\mu$ \DIFdelbegin \DIFdel{across species is negatively correlated with variation in }\DIFdelend \DIFaddbegin \DIFadd{covaries negatively with }\DIFaddend body mass, age at sexual maturity and longevity (\DIFdelbegin \DIFdel{$\rho=[-0.84, -0.83]$, }\DIFdelend table~\ref{tab:mammals_correlation}).
    ...
    Similarly, and more interestingly in the present context, \DIFdelbegin \DIFdel{the variation in }\DIFdelend $\Ne$ \DIFdelbegin \DIFdel{between species is also negatively correlated }\DIFdelend \DIFaddbegin \DIFadd{covaries negatively }\DIFaddend with {LHT}s (\DIFdelbegin \DIFdel{$\rho=[-0.54, -0.47]$, }\DIFdelend table~\ref{tab:mammals_correlation}).
    ...
    \DIFdelbegin \DIFdel{Result which }\DIFdelend \DIFaddbegin \DIFadd{This positive correlation between $\dnds$ and LHTs }\DIFaddend was also recovered on the present dataset, using a classical $\dnds$ based codon model (supplementary materials).
    Interestingly, the correlation \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{between }\DIFaddend $\dnds$ \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend LHTs is weaker than \DIFdelbegin \DIFdel{that of }\DIFdelend \DIFaddbegin \DIFadd{the correlation between }\DIFaddend our inferred $\Ne$ \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend LHTs, as expected if the variation in $\dnds$ indirectly (and imperfectly) reflects the underlying variation in \DIFdelbegin \DIFdel{Ne}\DIFdelend \DIFaddbegin \DIFadd{$\Ne$}\DIFaddend .
    Finally, $\Ne$ and $\mu$ are positively correlated in their variation ($\rho = 0.44$), which might simply reflect the fact that both \DIFdelbegin \DIFdel{negatively correlate }\DIFdelend \DIFaddbegin \DIFadd{covary negatively }\DIFaddend with {LHT}s.
    The partial-correlation coefficients (see supplementary) between $\Ne$ and {LHT}s are not significantly different from 0.
    However, this might simply be due to the very strong correlation between the three {LHT}s considered here\DIFdelbegin \DIFdel{($\rho=[0.81, 0.85]$)}\DIFdelend , such that controlling for any one of them removes most of the signal contributed by the \DIFdelbegin \DIFdel{available empirical }\DIFdelend \DIFaddbegin \DIFadd{empirically available }\DIFaddend variation between species.

	\begin{table*}[t]
     \DIFdelbeginFL %DIFDELCMD < \input{mammals/18CDS_SiteMutSelBranchNe_R1_TraitsCorrelation.tex}
%DIFDELCMD <     %%%
\DIFdelendFL \caption[Traits correlation in mammals]{
    		Correlation \DIFdelbeginFL \DIFdelFL{coefficient }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{coefficients }\DIFaddendFL between effective population size~($\Ne$), mutation rate per site per unit of time~($\mu$), and life-history traits (Maximum longevity, adult weight and female maturity).
    	\DIFdelbeginFL \DIFdelFL{Asterisks indicate strength of support of the }%DIFDELCMD < {%%%
\DIFdelFL{posterior}%DIFDELCMD < } %%%
\DIFdelFL{probability to be different than $0$ (pp) as $\smash{^{*}} pp > 0.95$ and $\smash{^{**}} pp > 0.975$.
        ...
        \noindent\adjustbox{max width=\textwidth}{%
            ...
            \end{tabu}}
        \flushleft\scriptsize{Asterisks indicate strength of support of the {posterior} probability to be different than $0$ (pp) as $\smash{^{*}} pp > 0.95$ and $\smash{^{**}} pp > 0.975$.}

        \DIFaddendFL \label{tab:mammals_correlation}
    \end{table*}
...
\DIFdelbegin \DIFdel{In our analysis across 4 concatenated random samples of }\DIFdelend \DIFaddbegin 
    ...
    \DIFadd{To assess the reproducibility of our inference, we analysed in total 6 different concatenated MSA each containing }\DIFaddend 12 \DIFdelbegin \DIFdel{genes, we observe a reproducible }\DIFdelend \DIFaddbegin \DIFadd{randomly sampled genes.
    The 6 different concatenated MSA showed similar trends in the change of $\mu$ and $\Ne$ between pairs of replicates }\DIFaddend (see supplementary)\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{.
    A statistical analysis performed on the pooled estimation of $\Ne$ across the 6 different concatenated MSA exhibits a }\DIFaddend statistically significant reduction in $\Ne$ for underground or depigmented species, or for species with visual impairment (see figure~\ref{fig:isopods_correlation}).
    Of note, the species that \DIFdelbegin \DIFdel{dit }\DIFdelend \DIFaddbegin \DIFadd{did }\DIFaddend not undergo a transition to subterranean environments feature a relative $\Ne$ close to $1$, meaning that $\Ne$ has not changed much along the lineages (since the root of the tree).
    ...
    %DIF <  redonner ici une evaluation du range de variation à travers la phylogénie.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Next, our empirical framework was also applied on a set of genes sampled across primates, taken from \citet{Perelman2011} and reanalysed in \citet{Brevet2019}.
    ...
    \DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{These }\DIFaddend results contrast with the finding of \citet{Brevet2019} on the same dataset based on $\dnds$-based {codon} models, where the estimated $\Ne$ was found to span several orders of magnitude, and correlated positively with $\ps$.
    ...
    \begin{figure*}[t]
        ...
        \end{minipage} \hfill
        \DIFdelbeginFL %DIFDELCMD < \llap{\raisebox{-1.2cm}{$p_{\text{value}}< 10^{-16}$, $r^2=0.18$\hspace{0.65cm}}}%%%
\DIFdelendFL \DIFaddbeginFL \llap{\raisebox{1.3cm}{\scriptsize \textbf{A}\hspace{4.2cm}}}\hfill
        \llap{\raisebox{-2.2cm}{$p_{\text{value}}< 10^{-16}$, $r^2=0.18$\hspace{0.65cm}}}\DIFaddendFL \hfill
        \begin{minipage}{0.32\linewidth}
            \includegraphics[width=\linewidth, page=1]{isopods/12CDS_SiteMutSelBranchNe_Rep_LogPopulationSize_pig_merged}
        \end{minipage} \hfill
        \DIFdelbeginFL %DIFDELCMD < \llap{\raisebox{-1.2cm}{$p_{\text{value}}< 10^{-16}$, $r^2=0.20$\hspace{0.64cm}}}%%%
\DIFdelendFL \DIFaddbeginFL \llap{\raisebox{1.3cm}{\scriptsize \textbf{B}\hspace{4.2cm}}}\hfill
        \llap{\raisebox{-2.2cm}{$p_{\text{value}}< 10^{-16}$, $r^2=0.20$\hspace{0.64cm}}}\DIFaddendFL \hfill
        \begin{minipage}{0.32\linewidth}
            \includegraphics[width=\linewidth, page=1]{isopods/12CDS_SiteMutSelBranchNe_Rep_LogPopulationSize_eye_merged}
        \end{minipage}
        \DIFdelbeginFL %DIFDELCMD < \llap{\raisebox{-1.2cm}{$p_{\text{value}}< 10^{-16}$, $r^2=0.20$\hspace{0.63cm}}}%%%
\DIFdelendFL \DIFaddbeginFL \llap{\raisebox{1.3cm}{\scriptsize \textbf{C}\hspace{4.2cm}}}\hfill
        \llap{\raisebox{-2.2cm}{$p_{\text{value}}< 10^{-16}$, $r^2=0.20$\hspace{0.63cm}}}\DIFaddendFL \hfill
        \caption[$\Ne$ as a function of traits in isopods]{
            $\Ne$ estimation for extant isopods species \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL sorted according to their habitat (left), pigmentation (middle), and ocular structure (right).
            \DIFaddbeginFL \DIFaddFL{Estimated $\Ne$ are pooled across 6 different multiple sequence alignments (MSA) each containing 12 randomly sampled genes in isopods species.
            }\DIFaddendFL All three qualitative \DIFdelbeginFL \DIFdelFL{trait }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{traits }\DIFaddendFL statistically correlate with changes in $\Ne$ \DIFdelbeginFL \DIFdelFL{.
        Underground, or depigmented species, or species with visual impairment are characteristic of low $\Ne$ species}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in the terminal branches}\DIFaddendFL .}
    ...
    Thus far, the development of mutation-selection models of the \DIFdelbegin %DIFDELCMD < {%%%
\DIFdel{HB}%DIFDELCMD < } %%%
...
\citet{Halpern1998} }\hspace{0pt}%DIFAUXCMD
    ...
    However, \DIFdelbegin \DIFdel{if the trends are in right direction }\DIFdelend \DIFaddbegin \DIFadd{although the changes in $\Ne$ are in the expected direction (negative correlation with body size, weight and maturity)~\mbox{%DIFAUXCMD
\citep{Romiguier2014}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , the magnitude of the changes inferred across the phylogeny is surprisingly narrow \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{(at most a factor $9.2$ in mammals).
    This range }\DIFaddend does not match independent empirical estimates of the variation in \DIFdelbegin \DIFdel{those clades.
In particular, in mammals, }\DIFdelend \DIFaddbegin \DIFadd{mammals, where }\DIFaddend {synonymous} diversity varies by a factor at least 10 between species~\citep{Galtier2016}.
...
\DIFdelbegin \DIFdel{In addition, the empirical fitting of the model requires more computing resources.
    ...
    First, \DIFaddbegin \DIFadd{the mutation rate ($\mu$) is considered site-invariant, an assumption which could be relaxed by introducing site-specific mutation rate to account for variation in mutation rate along the sequence.
}

    \DIFadd{Second, }\DIFaddend currently, our model also assumes no selection on {codon} usage.
    ...
    In principle, this assumption can be relaxed by implementing selective {codon} preferences that are shared across all sites\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{Nielsen2007}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
    ...
    \DIFdelbegin \DIFdel{Second, }\DIFdelend \DIFaddbegin \DIFadd{Third, providing a computationally more efficient implementation of the model would be important for broader application.
    Currently, running the program on an MSA of 18 mammals genes (77 extant species, and on }\DIFaddend the \DIFdelbegin \DIFdel{Bayesian analysis conducted here was based on relatively small alignments (20~000 sites at most), and with strong limits on the parametrization of the underlying mixture model (allowing for at most 50 distinct profile categories) .
Profiling of the program (not shown) shows that the number of components of the profile mixture is the limiting step of the computation.
    ...
    One possible development, leading to statistically more stable genome-wide estimates of $\Ne$, would be to develop a multi-gene parallelized version of the model, in which each coding sequence would have its own mixture model, and would run on a separate thread, while the history of }\DIFdelend \DIFaddbegin \DIFadd{order of 15000 nucleotide sites) for 4000 iterations of the chain (1000 are left as burn-in) takes approximately 2-4 weeks of computations, which is quite long although still accessible for reasonably small datasets.
    Increasing the computational efficiency could be achieved by several means: first, parallelizing the program could be achieved by dispatching genes over multiple cores.
    Second, a large fraction of the computing time is spent in updating the fitness profiles, and thus, fixing them to empirical values or using pre-estimated profiles under a constant }\DIFaddend $\Ne$ would \DIFdelbegin \DIFdel{be shared by all computing processes}\DIFdelend \DIFaddbegin \DIFadd{lead to a substantial acceleration}\DIFaddend .

    Finally, estimating $\Ne$ in a mutation-selection phylogenetic model relies on the relation between $\Ne$ and the relative strength of drift, in a context where, ultimately, the signal about the intensity of drift comes from the \DIFdelbegin \DIFdel{relative }\DIFdelend rate of {non-synonymous} substitutions \DIFaddbegin \DIFadd{relative to that of synonymous substitutions}\DIFaddend .
    ...
    In the model presented here, $\Ne$ and $\mu$ and quantitative traits are allowed to vary between species \DIFdelbegin \DIFdel{across branches}\DIFdelend \DIFaddbegin \DIFadd{(among branches) }\DIFaddend as a multivariate \DIFdelbegin \DIFdel{log-Brownian }\DIFdelend \DIFaddbegin \DIFadd{geometric Brownian }\DIFaddend process, but assumed constant along the DNA sequence.
    ...
    \begin{equation}
        ...
        \Exchan \sim \text{Dir}\left( \DIFdelbegin %DIFDELCMD < \dfrac{1}{6} %%%
\DIFdelend \DIFaddbegin \DIFadd{1}\DIFaddend ,\DIFdelbegin \DIFdel{6}\DIFdelend \DIFaddbegin \DIFadd{1,1,1,1,1 }\DIFaddend \right).
    ...
    \begin{equation}
        ...
        \Mutequi \sim \text{Dir}\left( \DIFdelbegin %DIFDELCMD < \dfrac{1}{4} %%%
\DIFdelend \DIFaddbegin \DIFadd{1}\DIFaddend ,\DIFdelbegin \DIFdel{4}\DIFdelend \DIFaddbegin \DIFadd{1,1,1 }\DIFaddend \right)
    ...
    \begin{figure*}[t]
        \centering
        \DIFdelbeginFL %DIFDELCMD < \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=0.6cm and 1.2cm,semithick]
%DIFDELCMD <         \tikzstyle{every state}=[]
%DIFDELCMD < 
...
%DIFDELCMD <         \path
%DIFDELCMD <         (Q) edge [black] node [above right] {\ref{eq:Probmatrix}} (P)
%DIFDELCMD <         (dT) edge [black] node [above left] {\ref{eq:branchlength}} (BL)
%DIFDELCMD <         (Mu) edge [black] node [] {} (BL)
%DIFDELCMD <         (BL) edge [black] node [] {} (P)
%DIFDELCMD <         (Ne) edge [black] node {} (Q)
%DIFDELCMD <         (R) edge [black] node {} (Q)
%DIFDELCMD <         (Bb) edge [black] node {} (Ne)
%DIFDELCMD <         (Bb) edge [black] node [below] {\ref{eq:branchNemu}} (Mu)
%DIFDELCMD <         (f) edge [black] node [above] {\ref{eq:subrates}} (Q)
%DIFDELCMD <         (Ex) edge [black] node [] {} (R)
%DIFDELCMD <         (Equi) edge [black] node [below] {\ref{eq:gtr-mutrates}} (R)
%DIFDELCMD <         (T) edge [black] node [above] {\ref{eq:ageTobranchtime}} (dT)
%DIFDELCMD <         (dT) edge [black] node {} (Bb)
%DIFDELCMD <         (Base) edge [black] node {} (f)
%DIFDELCMD <         (cat) edge [black] node [above] {\ref{eq:sitefitness}} (f)
%DIFDELCMD <         (ExH) edge [dashed, BLUE] node [above] {\ref{eq:DistribExchan}} (Ex)
%DIFDELCMD <         (EquiH) edge [dashed, BLUE] node [above] {\ref{eq:DistribMutequi}} (Equi)
%DIFDELCMD <         (Unif) edge [dashed, BLUE] node [above] {} (T)
%DIFDELCMD <         (Cov) edge [dashed, BLUE] node [above] {\ref{eq:DistribBrownian}} (Bb)
%DIFDELCMD <         (baseH) edge [dashed, BLUE] node [above] {\ref{eq:DistribBase}} (Base)
%DIFDELCMD <         (sb) edge [dashed, BLUE] node [above] {\ref{eq:DistribMultinomial}} (cat)
%DIFDELCMD <         (CovH) edge [dashed, BLUE] node [above] {\ref{eq:Distribcovariance}} (Cov)
%DIFDELCMD <         (sbH) edge [dashed, BLUE] node [above] {\ref{eq:DistribStickBreaking},\ref{eq:Beta}} (sb);
%DIFDELCMD <     \end{tikzpicture}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=0.6cm and 1.2cm,semithick]
...
\DIFaddendFL 
    ...
    Along a branch $\Setbranch$ of the tree, a \DIFdelbegin \DIFdel{log-Brownian }\DIFdelend \DIFaddbegin \DIFadd{geometric Brownian }\DIFaddend process starts at the oldest node at the tip of the branch~($\nodeUp$), and ends at the youngest node~($\nodeDown$).
    The rate of change of the \DIFdelbegin \DIFdel{log-Brownian }\DIFdelend \DIFaddbegin \DIFadd{geometric Brownian }\DIFaddend process per unit of time is constant and determined by the positive semi-definite and symmetric covariance matrix $\CovarianceMatrix$.
    Thus the distribution at node $\nodeDown$ of $\Brownian^{(\nodeDown)}$ is multivariate Gaussian, with mean equals to the Brownian process sampled at the oldest node $\Brownian^{(\nodeUp)}$, and variance $\branchtime\branchexp \CovarianceMatrix$:
    \begin{equation}
    ...
    All four simulators use a \DIFdelbegin \DIFdel{log-Brownian }\DIFdelend \DIFaddbegin \DIFadd{geometric Brownian }\DIFaddend multivariate process to model the changes in the mutation rate per generation, the generation time and $\Ne$ along the lineages.
    \texttt{SimuDiv}, \texttt{SimuFold} \& \texttt{SimuGeo} all simulate point substitutions along the phylogenetic tree.
    \DIFaddbegin \DIFadd{In our simulations, the tree is composed of 77 species (see supplementary), the tree root is $150$ million years old, the initial mutation rate is $\smash{10^{-8}}$ per site per generation and the initial generation time is $10$ years.
    }\DIFaddend The simulator starts from an initial sequence at equilibrium\DIFaddbegin \DIFadd{, composed of 15,000 codon sites}\DIFaddend .
    The change in fitness is computed for all possible \DIFdelbegin \DIFdel{mutant}\DIFdelend \DIFaddbegin \DIFadd{mutations}\DIFaddend , hence computing all strictly positive {substitution} rates.
    ...
    Mutations are drawn \DIFdelbegin \DIFdel{randomly based on mutation rates}\DIFdelend \DIFaddbegin \DIFadd{based on a user-defined nucleotide matrix, where our simulations used a symmetric time-reversible mutation matrix}\DIFaddend .
    ...
    Moreover, in \texttt{SimuPoly}, the instant value of log-$\Ne$ can also be modelled as a sum of a \DIFdelbegin \DIFdel{log-Brownian }\DIFdelend \DIFaddbegin \DIFadd{geometric Brownian }\DIFaddend process and an Ornstein-Uhlenbeck process.
    The \DIFdelbegin \DIFdel{log-Brownian }\DIFdelend \DIFaddbegin \DIFadd{geometric Brownian }\DIFaddend motion accounts for long-term fluctuations, while the Ornstein-Uhlenbeck introduces short-term fluctuations.
    ...
    Supplementary materials describe the \DIFdelbegin \DIFdel{models }\DIFdelend \DIFaddbegin \DIFadd{simulations }\DIFaddend in more details, \DIFaddbegin \DIFadd{with parameters and configurations used to produce alignments, }\DIFaddend as well as performance of the inference model against them.

    \subsection{Empirical data}
    For placental mammals, alignments were extracted from OrthoMam database~\citep{Ranwez2007,Scornavacca2019}.
    \DIFdelbegin \DIFdel{Only highly conserved coding sequences are kept for the analysis, representing 226 }%DIFDELCMD < {%%%
\DIFdel{CDS}%DIFDELCMD < } %%%
\DIFdel{with $\leq 1\%$ of gaps in the alignment.
}\DIFdelend Life-history traits ({LHT}s) for longevity, age at maturity and weight were obtained from AnAge database~\citep{DEMAGALHAES2009,Tacutu2012}.
    We focused our analysis on 77 taxa for which information is available for at least one {LHT}.
    \DIFaddbegin \DIFadd{The list of conserved genes putatively not under positive selection is available in supplementary materials.
}\DIFaddend 
    ...
    \DIFaddbegin \DIFadd{We thank the editor and three anonymous reviewers for their constructive comments, which helped us to improve the manuscript and the analysis.
    }\DIFaddend This work was performed using the computing facilities of the CC LBBE/PRABI.

    \DIFaddbegin \bibliography{references,introduction}
    \DIFaddend \bibliographystyle{natbib}%%%%Bibliography style file
 \DIFdelbegin %DIFDELCMD < \bibliography{references,introduction}
%DIFDELCMD <     
