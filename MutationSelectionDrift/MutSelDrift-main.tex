...
Instead, mechanistic \gls{codon} models explicitly introduced population-genetics equations into phylogenetic \gls{codon} models~\citep{Halpern1998}.
...
Formally, DNA coding sequences are modeled at the level of the \gls{codon}, where for each codon site (triplet of nucleotide), a change of \gls{codon} means a change in \acrshort{DNA} but not necessarily in amino-acid.
...
Our Bayesian implementation, written in C++ in the software \textit{BayesCode}, is publicly available at \url{https://github.com/bayesiancook/bayescode/tree/chronogram}.
...
Additionally, under a Fisher geometric landscape developed in supplementary materials (section~\ref{subsec:fisher-geometric-landscape}) and also incorporating epistasis, branch length is reliably estimates while $\Ne$ is also more difficult to estimate.
...
Protein coding DNA sequences alignment and life-history traits such as habitat (surface or underground), pigmentation (depigmented, partially depigmented or pigmented) and ocular structure (anophthalmia, microphtalmia, or ocular) are available for these species~\citep{Scalier2019}.
Estimation of $\Ne$ reveals a correlation between life-history traits and $\Ne$, presented section \ref{sec:isopods} in supplementary materials.
However these estimation are sensitive to phylogenetic inertia since not obtained under a comparative, although the phylogenetic inertia.
...
Third, the \acrshort{DNA} sequences could also be misaligned in some sites, however we observe the same magnitude of inferred $\Ne$ for different set of genes indicating this might not be the primary reason.
...
Fitness profiles estimated are averaged along the phylogeny and are more seemingly \gls{neutral} than our estimated profiles under our present framework (see table \ref{tab:table-entropy-aa-mutselne} in supplementary materials).
...
\label{sec:Bayesian}
Monte-Carlo Markov-Chain (\acrshort{MCMC}) are run for 4000 points and the first 1000 points are discarded as burn-in, the convergence is then assessed as shown supplementary materials (section \label{subsec:chain-convergence}), as both site-specific fitness and branch $\Ne$ have the same posterior mean.
