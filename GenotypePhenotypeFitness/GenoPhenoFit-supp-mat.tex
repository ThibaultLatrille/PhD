\section{\texorpdfstring{$\avgpfix$}{φ} susceptibility after a change in \texorpdfstring{$\Ne$}{Nₑ}}
...
\subsection{Mean scaled fixation probability (\texorpdfstring{$\avgpfix$}{φ}) at equilibrium}
The mean fixation probability relative to the fixation probability of \gls{neutral} mutations, also called the mean scaled fixation probability ($\avgpfix$) is by definition:
\begin{align}
...
\end{align}
$\avgpfix\eq$ at equilibrium is then determined by the \gls{Phenotype} at equilibrium $x\eq$:
\begin{align}
...
\end{center}
Moreover, given that the number of state if large enough $\Nstate \gg 1$, the equilibrium $\avgpfix$ can be approximated as:
\begin{align}
...
\end{center}
Moreover, given that the number of state if large enough $\Nstate \gg 1$, the response in equilibrium $\avgpfix$ due to change in \gls{Phenotype} can be approximated as:
\begin{align}
...
\end{align}
\subsection{\texorpdfstring{$\avgpfix$}{φ} susceptibility after a change in \texorpdfstring{$\Ne$}{Nₑ}}
...
In \citet{Brevet2019}, the covariance matrix with $\ln(\Ne)$ and $\ln(\avgpfix)$ as entries allows to approximate $\chi$:
\begin{align}
	\widehat{\frac{\der \ln (\avgpfix)}{\der \ln (\Ne)}} & = \dfrac{\text{Cov}[\ln(\avgpfix), \ln(\Ne)]}{\text{Var}[\ln(\Ne)]}\text{ from linear regression.} \\
	\Rightarrow \widehat{\frac{\der \avgpfix}{\avgpfix \der \ln (\Ne)}} & = \dfrac{\text{Cov}[\ln(\avgpfix), \ln(\Ne)]}{\text{Var}[\ln(\Ne)]} \\
	\Rightarrow \widehat{\chi} & \simeq \widehat{\avgpfix} \dfrac{\text{Cov}[\ln(\avgpfix), \ln(\Ne)]}{\text{Var}[\ln(\Ne)]} \\
...
\section{Simulated \texorpdfstring{$\avgpfix$}{φ} susceptibility to changes in \texorpdfstring{$\Ne$}{Nₑ}}
