...
\subsection{Mean scaled fixation probability (\texorpdfstring{$\omega$}{ω}) at equilibrium}
The mean fixation probability relative to the fixation probability of neutral mutations, also called the mean scaled fixation probability ($\omega$) is by definition:
\begin{align}
...
\begin{align}
\frac{ \der \omega\eq}{\der \ln (\Ne)} & \simeq - \frac{\frac{ \partial \logfit (x\eq) }{\partial {x\eq}}}{\frac{ \partial^2 \logfit (x\eq) }{\partial {x\eq}^2}}\label{eq:approx_dw_dlnNe}
\end{align}
...
\begin{align}
P_{F} (x) & = \frac{1}{1 + e^{\beta(\DeltaGmin + \DeltaDeltaG \Nsite x)}}
\end{align}
...
Thus, $\DeltaGmin < 0$ is the difference in free energy between folded and unfolded state when all sites are stable. $\Nsite \DeltaDeltaG$ is the expected change in $\DeltaG$ when all sites are unstable.
The misfolded fraction is  $P_{U} = 1-P_{F}$. In addition, $P_{F}$ is typically close to 1 (or $P_{U} \ll 1$), so that we can use a first-order approximation:
\begin{align}
P_{F}(x) & = 1 - P_{U}(x) \\
& \simeq 1 - e^{\beta(\DeltaGmin + \DeltaDeltaG \Nsite x)}
\end{align}
or equivalently
\begin{align}
P_{U}(x) & \simeq e^{\beta(\DeltaGmin + \DeltaDeltaG \Nsite x)}
\end{align}
...
\begin{gather}
 \wrightfit (x) = \frac{1}{1 + \e^{\beta(\DeltaGmin + \Nsite \DeltaDeltaG x)}}. \label{eq:fitness}
\end{gather}
The derivative of fitness w.r.t to phenotype is:
\begin{align}
\frac{\partial  \logfit (x)}{\partial x}  & = - \frac{\partial \ln \left( 1 + \e^{\beta(\DeltaGmin + \Nsite \DeltaDeltaG x)} \right)}{\partial x} \text{ from eq.~\ref{eq:fitness}}, \\
& = - \beta \Nsite \DeltaDeltaG \frac{\e^{\beta(\DeltaGmin + \Nsite \DeltaDeltaG x)}}{1 + \e^{\beta(\DeltaGmin + \Nsite \DeltaDeltaG x)}} \label{eq:df_dx},\\
& \simeq - \beta \Nsite \DeltaDeltaG \e^{\beta(\DeltaGmin + \Nsite \DeltaDeltaG x)}.
\end{align}
The equilibrium phenotype ($x\eq$) is :
\begin{gather}
\lambda_{K}(x\eq) = 4\Ne \beta \DeltaDeltaG \frac{\e^{\beta(\DeltaGmin + \Nsite \DeltaDeltaG x\eq)}}{1 + \e^{\beta(\DeltaGmin + \Nsite \DeltaDeltaG x\eq)}}  \text{ from eq.~\ref{eq:equilibrium_lambda} and~\ref{eq:df_dx}}.
\end{gather}
Using $\Ne=10^4$, $\beta=1.686$, $\DeltaGmin = -118$, $\Nsite=300$, $\DeltaDeltaG=1$, we have the following :
\begin{center}
...
\begin{align}
\frac{\partial^2  \logfit (x)}{\partial x^2} & = - \beta \Nsite \DeltaDeltaG \frac{\partial}{\partial x} \left( \frac{\e^{\beta(\DeltaGmin + \Nsite \DeltaDeltaG x)}}{1 + \e^{\beta(\DeltaGmin + \Nsite \DeltaDeltaG x)}} \right) \text{ from eq.~\ref{eq:df_dx}}, \\
 & = - \beta \Nsite \DeltaDeltaG  \beta \Nsite \DeltaDeltaG \frac{\e^{\beta(\DeltaGmin + \Nsite \DeltaDeltaG x)}}{\left( 1 + \e^{\beta(\DeltaGmin + \Nsite \DeltaDeltaG x)}\right)^2}, \\
 & = \frac{\beta \Nsite \DeltaDeltaG}{1 + \e^{\beta(\DeltaGmin + \Nsite \DeltaDeltaG x)}} \frac{ \partial \logfit (x) }{\partial x} \text{ from eq.~\ref{eq:df_dx}} \label{eq:df2_dx2}, \\
 & \simeq \beta \Nsite \DeltaDeltaG \frac{ \partial \logfit (x) }{\partial x} 
\end{align}
Finally, $\omega$ susceptibility after a change in $\Ne$ is simply:
\begin{align}
\frac{ \der \omega\eq}{\der \ln (\Ne)} & \simeq - \frac{1}{\beta \Nsite \DeltaDeltaG} \text{from eq.~\ref{eq:df2_dx2} and~\ref{eq:approx_domega_dx}},
\end{align}
...
\begin{align}
\frac{\partial  \logfit (x)}{\partial x} & \simeq -A y \beta \DeltaDeltaG \Nsite e^{\beta(\DeltaGmin + \DeltaDeltaG \Nsite x)}.
\end{align}
Under this model, the phenotype at equilibrium is given by:
\begin{align}
\lambda_{K}(x\eq) = 4\Ne y A \beta \DeltaDeltaG \Nsite e^{\beta(\DeltaGmin + \DeltaDeltaG \Nsite x\eq)} \text{ from eq.~\ref{eq:equilibrium_lambda} and~\ref{eq:df_dx}}.
\end{align}
And the susceptibility of $\omega$ after a change in $\Ne$ is the same as before:
\begin{align}
\frac{ \der \omega\eq}{\der \ln (\Ne)} & \simeq - \frac{1}{\beta \Nsite \DeltaDeltaG}.
\end{align}
...
\begin{align}
\frac{ \der \omega\eq}{\der \ln (y)} = \frac{ \der \omega\eq}{\der \ln (\Ne)} \simeq - \frac{1}{\beta \Nsite \DeltaDeltaG}.
\end{align}
...
\begin{align}
\logfit (x) & = - A y \sum_k \pi_k e^{\beta(\DeltaGmin + \DeltaDeltaG\Nsite x + \DeltaDeltaG k)}
\\
& = - A y  e^{\beta(\DeltaGmin + \DeltaDeltaG\Nsite x)} \sum_k e^{-\rho n} \frac{(\rho n)^k}{k!} e^{\beta \DeltaDeltaG k}
\\
& = - A y  e^{\beta(\DeltaGmin + \DeltaDeltaG\Nsite x) + \rho \Nsite (e^{\beta \DeltaDeltaG} -1 )}
\\
& \simeq - A y  e^{\beta(\DeltaGmin + \DeltaDeltaG\Nsite x) + \rho \beta \DeltaDeltaG \Nsite}
\\
& =- A y  e^{\beta(\DeltaGmin + \DeltaDeltaG \Nsite (x + \rho))}
\end{align}
...
\begin{align}
4 \Ne \frac{\partial  \logfit (x)}{\partial x} & \propto -4 \Ne y e^{\beta(\DeltaGmin + \DeltaDeltaG \Nsite (x + \rho))}
\\ & \propto -4 \Ne y e^{\beta(\DeltaGmin + \DeltaDeltaG\Nsite x)}
\end{align}
...
\begin{align}
\logfit (x) & = -A \frac{y}{B} \left( 1 + e^{\beta(\DeltaGmin + \DeltaDeltaG\Nsite x)} \right)
\\
& = -by (1 + e^{\beta(\DeltaGmin + \DeltaDeltaG\Nsite x)})
\end{align}
...
\begin{align}
4 \Ne \frac{\partial  \logfit (x)}{\partial x} & \propto -4 \Ne y e^{\beta(\DeltaGmin + \DeltaDeltaG\Nsite x)}
\end{align}
...
\begin{align}
4 \Ne \frac{\partial  \logfit (x)}{\partial x} & \propto -4 \Ne g(y) e^{\beta(\DeltaGmin + \DeltaDeltaG\Nsite x)}
\end{align}
...
\\
& = \DeltaGmin + \DeltaDeltaG \Nsite (x_i + x_j)
\end{align}
...
\begin{align}
C & = [i] + (m-1) [i] \frac{C}{C_0} \, e^{\beta (\DeltaGmin + \DeltaDeltaG \Nsite (\bar x + x_i))}
\end{align}
...
\begin{align}
[i] & = \frac{C} {1 + (m-1) \frac{C}{C_0} \, e^{\beta (\DeltaGmin + \DeltaDeltaG \Nsite (\bar x + x_i))}}
\\ & \simeq C \left( 1 - m \frac{C}{C_0} \, e^{\beta (\DeltaGmin + \DeltaDeltaG \Nsite (\bar x + x_i))} \right)
\\ & =
C (1 - \epsilon_i)
\end{align}
and thus $\epsilon_i$ can be identified with:
\begin{align}
\epsilon_i  & = m \frac{C}{C_0} \, e^{\beta (\DeltaGmin +\DeltaDeltaG \Nsite (\bar x + x_i))}
\end{align}
...
\\ & =
b \beta \DeltaDeltaG \Nsite m \frac{C}{C_0} \, e^{\beta (\DeltaGmin +\DeltaDeltaG \Nsite (\bar x + x_i))}
\end{align}
and thus:
\begin{align}
\ln \kappa_i & = \ln \left( b \beta \DeltaDeltaG \Nsite m \frac{C}{C_0}  \right) \, + \, \beta (\DeltaGmin +\DeltaDeltaG \Nsite \bar x) \, + \, \beta \DeltaDeltaG\Nsite x_i
\end{align}
...
\\ & =  2 (\lambda - 1) \, \frac{1}{\beta \DeltaDeltaG n}
\end{align}
...
\begin{figure}[H]
	...
	For each population size, $200$ simulations were performed and the average (solid line) and $90\%$ confidence interval (shaded area) are shown.
	The fixed parameters are $\DeltaGmin=-118$, $\DeltaDeltaG=1$, $\Nsite=300$, $\beta=1.686$.
	The simulations of our additive free energy model match the theoretical prediction that the slope of the linear relation (dashed line) is equal to $\beta \Nsite \DeltaDeltaG)^{-1} = 0.00198 \simeq = 0.00199$.
...
\begin{figure}[H]
	...
	For each population size, $200$ simulations were performed and the average (solid line) and $90\%$ confidence interval (shaded area) are shown.
	The fixed parameters are $\DeltaGmin=-118$, $\DeltaDeltaG=1$, $\Nsite=300$, and for each non-optimal amino acid, $\DeltaDeltaG$ is scaled by the Grantham distance to the optimal amino acid. $\beta$ are given in the legend.
...
\begin{figure}[H]
	...
	For each population size, $200$ simulations were performed and the average (solid line) and $90\%$ confidence interval (shaded area) are shown.
	The fixed parameters are $\DeltaGmin=-118$, $\DeltaDeltaG=1$, $\beta=1.686$, and for each non-optimal amino acid, $\DeltaDeltaG$ is scaled by the Grantham distance to the optimal amino acid. $\Nsite$ are given in the legend.
...
\begin{figure}[H]
	...
	For each population size, $200$ simulations were performed and the average (solid line) and $90\%$ confidence interval (shaded area) are shown.
	The fixed parameters are $\DeltaGmin=-118$, $\beta=1.686$, $\Nsite=300$, and for each non-optimal amino acid, $\DeltaDeltaG$ is scaled by the Grantham distance to the optimal amino acid. $\DeltaDeltaG$ are given in the legend.
...
\begin{figure}[H]
	...
	For each population size, $200$ simulations were performed and the average (solid line) and $90\%$ confidence interval (shaded area) are shown.
	The parameters are $\DeltaGmin=-118$, $\Nsite=300$, $\beta=1.686$ and each site has its own gamma distributed $\DeltaDeltaG$ with mean $1$ and standard deviation given in the legend. $\DeltaDeltaG$ is scaled by the Grantham distance to the optimal amino acid.
...
\begin{figure}[H]
	...
	For each population size, $200$ simulations were performed and the average (solid line) and $90\%$ confidence interval (shaded area) are shown.
	The parameters are $\DeltaGmin=-118$, $\Nsite=300$, $\beta=1.686$ and each site has its own gamma distributed $\DeltaDeltaG$ with mean $1$ and standard deviation given in the legend.
	Increasing the variance of $\DeltaDeltaG$ increases $\omega$, by shifting the equilibrium to higher $x\eq$ since more unstable sites with low $\DeltaDeltaG$ are fixed before reaching sensible deleterious selection coefficient against unstable mutations. Once many sites are unstable, the $\omega$ is higher since non-synonymous mutations between unstable states are effectively neutral. However the slope of the $\omega$-$\Ne$ relationship is not sensibly changed.
	}
\end{figure}
...
\begin{figure}[H]
	...
	\caption[Relaxation time of $\omega$ dependence on $\Nsites$, while correction for $\DeltaGmin$]{
	$\omega$ Relaxation after a brutal change in $\Ne$, for various $\Nsites$ while correcting for $\DeltaGmin$.
	...
	$\beta=1.686$, $\DeltaDeltaG=0.2$ for all simulations. The number of sites is changed from $\Nsite=15$ to $\Nsite=158$, and the number of replicates is changed accordingly such that the total number of sites ($\Nsite*r$) is kept constant.
	Moreover, $\DeltaGmin$ is changed according to $\Nsite$ and $\DeltaDeltaG$ such that the equilibrium value $x\eq$ is kept constant, by solving numerically equation~\ref{eq:equilibrium-pheno}.
...
\begin{figure}[H]
	...
	Solid line corresponds to the average over replicates ($r$) and the shaded area correspond to the $90\%$ interval among replicates.
	The mutation rate ($\mu$) is $1e{-8}$ per year per site, and the total time of the computation is $900$ million years.
	$\beta=1.686$, $\DeltaDeltaG=0.2$ and $\DeltaGmin=-10$ for all simulations. The number of sites is changed from $\Nsite=15$ to $\Nsite=158$, and the number of replicates is changed accordingly such that the total number of sites ($\Nsite*r$) is kept constant.
...
\begin{figure}[H]
	...
	\caption[Relaxation time of $\omega$ dependence on $\Nsites$, while correction for $\DeltaDeltaG$]{
	$\omega$ Relaxation after a brutal change in $\Ne$, for various $\Nsites$ while correcting for $\DeltaDeltaG$.
	The left and right panel correspond to low $\Ne$ ($1e^{5}$) and the middle panel corresponds to high $\Ne$ ($2e^{6}$).
	Solid line corresponds to the average over replicates ($r$) and the shaded area correspond to the $90\%$ interval among replicates.
	The mutation rate ($\mu$) is $1e{-8}$ per year per site, and the total time of the computation is $900$ million years.
	$\beta=1.686$, $\DeltaGmin=-10$ for all simulations. The number of sites is changed from $\Nsite=15$ to $\Nsite=158$, and the number of replicates is changed accordingly such that the total number of sites ($\Nsite*r$) is kept constant.
	Moreover, $\DeltaDeltaG$ is changed according to $\Nsite$ such that the product $\DeltaDeltaG\Nsite$ is kept constant, thus the  susceptibility of the $\omega$ to changes in $\Ne$ is kept constant.
...
\begin{figure}[H]
	...
	Solid line corresponds to the average over replicates ($r$) and the shaded area correspond to the $90\%$ interval among replicates.
	The mutation rate ($\mu$) is $1e{-8}$ per year per site, and the total time of the computation is $900$ million years.
	$\beta=1.686$, $\DeltaDeltaG=-10$ for all simulations. The number of sites is changed from $\Nsite=15$ to $\Nsite=158$, and the number of replicates is changed accordingly such that the total number of sites ($\Nsite*r$) is kept constant.
	Moreover, $\DeltaDeltaG$ is changed according to $\Nsite$ such that the product $\DeltaDeltaG\Nsite$ is kept constant, thus the  susceptibility of the $\omega$ to changes in $\Ne$ is kept constant.
	Finally, $\DeltaGmin$ is changed according to $\Nsite$ and $\DeltaDeltaG$ such that the equilibrium value $x\eq$ is kept constant, by solving numerically equation~\ref{eq:equilibrium-pheno}.
	Increasing $\Nsite$ implies a higher rate of relaxation.}
\end{figure}
...
\begin{figure}[H]
	...
	$\DeltaGmin=-118$, $\DeltaDeltaG=1$, $\Nsite=300$, $\beta=1.686$, and for each non-optimal amino acid, $\DeltaDeltaG$ is scaled by the Grantham distance to the optimal amino acid.
