...
%DIF DEL main-elsevier.tex   Wed Jan 27 12:33:59 2021
%DIF ADD main-revision.tex   Mon Sep  6 16:48:45 2021
%DIF < \documentclass{elsarticle}
%DIF -------
\documentclass[authoryear]{elsarticle} %DIF > 
%DIF -------
...
\newcommand{\ATtoGC}{\text{AT} \rightarrow \text{GC}} %DIF > 
\newcommand{\GCtoAT}{\text{GC} \rightarrow \text{AT}} %DIF > 
...
% Tree %DIF > 
\newcommand{\Tree}{\mathcal{T}} %DIF > 
\newcommand{\state}{\mathcal{S}} %DIF > 
\newcommand{\node}{\text{n}} %DIF > 
\newcommand{\taxon}{\node} %DIF > 
\newcommand{\Settaxon}{\taxon \in \{1, \hdots, \Ntaxa \}} %DIF > 
\newcommand{\Ntaxa}{P} %DIF > 
\newcommand{\treeroot}{0} %DIF > 
\newcommand{\treerootexp}{^{(\treeroot)}} %DIF > 
\newcommand{\branch}{\text{b}} %DIF > 
\newcommand{\branchexp}{^{(\branch)}} %DIF > 
\newcommand{\Nbranch}{2 \Ntaxa - 2} %DIF > 
\newcommand{\Setbranch}{\branch \in \{1, \hdots, 2 \Ntaxa - 2\}} %DIF > 
\newcommand{\nodeUp}{\branch^{\uparrow}} %DIF > 
\newcommand{\nodeDown}{\branch^{\downarrow}} %DIF > 
\newcommand{\nodeexp}{^{(\node)}} %DIF > 
\newcommand{\Nnode}{2 \Ntaxa - 2} %DIF > 
\newcommand{\Ninternal}{\Ntaxa - 2} %DIF > 
\newcommand{\Setnode}{\node \in \{\treeroot, \hdots, 2 \Ntaxa - 2\}} %DIF > 
\newcommand{\Setnodenoroot}{\node \in \{1, \hdots, 2 \Ntaxa - 2\}} %DIF > 
\newcommand{\Setinternal}{\node \in \{\Ntaxa + 1, \hdots, 2 \Ntaxa - 2\}} %DIF > 
\newcommand{\branchnode}{\mathcal{W}} %DIF > 
\newcommand{\uniform}{0, 1} %DIF > 
\newcommand{\age}{T} %DIF > 
\newcommand{\branchtime}{\Delta \age} %DIF > 
\newcommand{\branchlength}{l} %DIF > 
% Alignment %DIF > 
\newcommand{\data}{D} %DIF > 
\newcommand{\Data}{\BiDimArray{\data}} %DIF > 
\newcommand{\site}{\text{z}} %DIF > 
\newcommand{\Nsite}{\text{Z}} %DIF > 
\newcommand{\siteexp}{^{(\site)}} %DIF > 
\newcommand{\Nstate}{\text{K}} %DIF > 
\newcommand{\Setsite}{\site \in \{1, \hdots, \Nsite\} } %DIF > 
\newcommand{\sumSetsite}{_{\site=1}^{\Nsite} } %DIF > 
\newcommand{\branchsiteexp}{^{(\branch, \site)}} %DIF > 
\newcommand{\treerootsiteexp}{^{(\treeroot, \site)}} %DIF > 
\newcommand{\taxonsiteexp}{^{(\taxon, \site)}} %DIF > 
% Categories %DIF > 
\newcommand{\cat}{\text{k}} %DIF > 
\newcommand{\Ncat}{\text{K}} %DIF > 
\newcommand{\catexp}{^{(\cat)}} %DIF > 
\newcommand{\catInterval}{\{1, \hdots, \Ncat\}} %DIF > 
\newcommand{\Setcat}{\cat \in \catInterval } %DIF > 
\newcommand{\branchcatexp}{^{(\branch, \cat)}} %DIF > 
\newcommand{\treerootcatexp}{^{(\treeroot, \cat)}} %DIF > 
\newcommand{\profile}{\phi} %DIF > 
\newcommand{\Profile}{\UniDimArray{\profile}} %DIF > 
\newcommand{\concentrationProfile}{\alpha} %DIF > 
\newcommand{\centerProfile}{\UniDimArray{\gamma}} %DIF > 
\newcommand{\catVar}{\kappa} %DIF > 
\newcommand{\catsite}{\catVar\left(\site\right)} %DIF > 
\newcommand{\catmultivar}{m} %DIF > 
\newcommand{\catMultiVar}{\UniDimArray{\catmultivar}} %DIF > 
\newcommand{\stickbreaking}{\theta} %DIF > 
\newcommand{\StickBreaking}{\UniDimArray{\stickbreaking}} %DIF > 
\newcommand{\stick}{\psi} %DIF > 
\newcommand{\stickbreakinghyper}{\beta} %DIF > 
% Traits %DIF > 
\newcommand{\traiti}{a} %DIF > 
\newcommand{\traitj}{b} %DIF > 
\newcommand{\trait}{X} %DIF > 
\newcommand{\Trait}{\UniDimArray{\trait}} %DIF > 
\newcommand{\Ntrait}{L} %DIF > 
\newcommand{\traitInterval}{\{1, \hdots, \Ntrait\}} %DIF > 
\newcommand{\lhtInterval}{\{3, \hdots, \Ntrait\}} %DIF > 
\newcommand{\Settrait}{\traiti \in \traitInterval} %DIF > 
\newcommand{\contrast}{\UniDimArray{C}} %DIF > 
\newcommand{\Covariancematrix}{\Sigma} %DIF > 
\newcommand{\CovarianceMatrix}{\BiDimArray{\Covariancematrix}} %DIF > 
\newcommand{\Precisionmatrix}{\Omega} %DIF > 
\newcommand{\PrecisionMatrix}{\BiDimArray{\Precisionmatrix}} %DIF > 
\newcommand{\Identitymatrix}{\BiDimArray{I}} %DIF > 
\newcommand{\brownian}{B} %DIF > 
\newcommand{\Brownian}{\UniDimArray{\brownian}} %DIF > 
\newcommand{\covariancedf}{q} %DIF > 
\newcommand{\covariancekappa}{\kappa} %DIF > 
\newcommand{\Scattermatrix}{\BiDimArray{A}} %DIF > 
% Random variables %DIF > 
\newcommand{\pruning}{\psi} %DIF > 
\newcommand{\Multivariate}{\UniDimArray{Z}} %DIF > 
\newcommand{\subhistory}{\mathcal{H}} %DIF > 
 %DIF > 
% Genotype-Phenotype-Fitness %DIF > 
\newcommand{\phenoGeo}{\UniDimArray{P}} %DIF > 
\newcommand{\phenoFold}{\Delta G} %DIF > 
\newcommand{\eq}{^{*}} %DIF > 
\newcommand{\NbrSites}{n} %DIF > 
\newcommand{\dx}{\delta x} %DIF > 
\newcommand{\Fold}{\text{F}} %DIF > 
\newcommand{\Unfold}{\text{U}} %DIF > 
\newcommand{\probaFold}{\proba_{\Fold}} %DIF > 
\newcommand{\probaUnfold}{\proba_{\Unfold}} %DIF > 
\newcommand{\G}{G} %DIF > 
\newcommand{\GFold}{\G_{\Fold}} %DIF > 
\newcommand{\GUnfold}{\G_{\Unfold}} %DIF > 
\newcommand{\DeltaG}{\Delta \G} %DIF > 
\newcommand{\DeltaGmin}{\alpha} %DIF > 
\newcommand{\DeltaDeltaG}{\gamma} %DIF > 
\newcommand{\EmpiricalDeltaDeltaG}{\Delta \DeltaG} %DIF > 
\newcommand{\EmpiricalDeltaGmin}{\DeltaG_{\text{min}}} %DIF > 
...
\DIFdelbegin \DIFdel{The study of protein-coding sequences evolution fostered another modelling approach, based on genotype-fitness maps instead of distribution of fitness effects.
In this }\DIFdelend \DIFaddbegin \DIFadd{However, the DFE may not be fixed a priori, instead it may naturally emerge from an underlying fitness landscape and thus vary depending on the position of the population in the fitness landscape~\mbox{%DIFAUXCMD
\citep{Lourenco2013}}\hspace{0pt}%DIFAUXCMD
.
In an }\DIFaddend alternative approach, \DIFaddbegin \DIFadd{based on genotype-fitness maps modelling the underlying fitness landscape, }\DIFaddend the selective effect of a mutation depends on the fitness of both the source and the target amino acids involved in the mutation event~\citep{Halpern1998, Rodrigue2010, Tamuri2012}.
...
Conversely, one striking theoretical result was \DIFdelbegin \DIFdel{the proof }\DIFdelend that $\omega$ is in fact predicted to be independent of $\Ne$\DIFdelbegin \DIFdel{under relatively general circumstances, namely}\DIFdelend , whenever (i) the fitness is a log-concave function of a {phenotype} and (ii) the {phenotype} itself is equimutable.
...
This general theoretical argument \DIFaddbegin \DIFadd{was obtained using a heuristic derivation in the deterministic limit, that is, ignoring the fluctuations at stationarity.
It }\DIFaddend has been invoked in the context of \textit{in silico} experiments of protein sequence evolution, assuming that proteins are under selection for their thermodynamic stability, with fitness being proportional to the folding probability of the protein~\citep{Goldstein2013}.
...
To this effect, we develop a general mathematical approach \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{assuming a given genotype-phenotype-fitness map.
We determine an approximate solution to the equilibrium phenotype, as in \mbox{%DIFAUXCMD
\citet{Charlesworth2013}}\hspace{0pt}%DIFAUXCMD
, which is then related to the substitution rate $\omega$.
Subsequently, we }\DIFaddend derive a quantitative approximation of the response of $\omega$ to changes in $\Ne$ and expression level, \DIFdelbegin \DIFdel{in the context of a given genotype-phenotype-fitness map, }\DIFdelend as depicted in figure~\ref{fig:Summary}.
\DIFaddbegin \DIFadd{We apply our mathematical approach to fitness models based on protein thermodynamics.
}\DIFaddend In the light of previously published empirical estimates from protein thermodynamics and comparative genomics, we discuss the articulation between empirical data and our mechanistic model.
...
Analytically, the fitness function is given by the Fermi Dirac distribution\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{Goldstein2011} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend and is typically close to $1$\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{Serohijos2014}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , leading to a first-order approximation\DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{Goldstein2011}}\hspace{0pt}%DIFAUXCMD
:
...
To derive the probabilities of such events to occur, we \DIFaddbegin \DIFadd{assume that the state of each site is one of the 20 amino-acids, considered equivalent, and }\DIFaddend also make the simplifying assumption that all transitions between \DIFaddbegin \DIFadd{the }\DIFaddend amino acids are equiprobable.
Altogether, any mutation in the sequence can then have a phenotypic effect of $0$ or $\dx=1 /\NbrSites$, with probabilities of transitions \DIFaddbegin \DIFadd{from $x$ to $x'$ }\DIFaddend equal to:
\begin{gather}
    \DIFdelbegin %DIFDELCMD < \begin{cases}
%DIFDELCMD <         \dx &\text{ with probability } 1-x, \\
%DIFDELCMD <         0 &\text{ with probability } \frac{18 x }{19}, \\
%DIFDELCMD <         -\dx &\text{ with probability } \frac{x}{19}.\\
%DIFDELCMD <     \end{cases}%%%
\DIFdelend \DIFaddbegin \begin{cases}
        \proba ( x' = x + \dx ) = 1-x, \\
        \proba ( x' = x ) = \frac{18 x }{19}, \\
        \proba ( x' = x -\dx ) = \frac{x}{19}.\\
    \end{cases}\DIFaddend  \label{eq:proba}
\end{gather}
...
\DIFdelbegin \DIFdel{This equilibrium }\DIFdelend \DIFaddbegin \DIFadd{Under the assumption that fluctuations around the equilibrium value are small (in supplementary materials), the equilibrium distribution of trait can be approximated by an equilibrium trait value, thus using a deterministic approximation, as in \mbox{%DIFAUXCMD
\citet{Cherry1998}}\hspace{0pt}%DIFAUXCMD
.
This equilibrium value }\DIFaddend can be determined by expressing the constraint that the selection coefficient of {substitutions} is expected to be null on average~\citep{Goldstein2013}.
...
\begin{figure*}[h]
    \centering
    \includegraphics[width=\textwidth, page=1] {figure-2.pdf}

    \DIFdelbeginFL %DIFDELCMD < \caption[Response of the equilibrium phenotype after a change in $\Ne$]{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Response of the equilibrium phenotype after a change ineffective population size]{\DIFaddendFL 
        Response of the equilibrium {phenotype} \DIFaddbeginFL \DIFaddFL{($x\eq$) }\DIFaddendFL after a change in \DIFaddbeginFL \DIFaddFL{effective population size~(}\DIFaddendFL $\Ne$\DIFaddbeginFL \DIFaddFL{)}\DIFaddendFL .
        The equilibrium {phenotype} $x\eq$ is obtained when the selective pressure equals to the mutational pressure (\DIFaddbeginFL \DIFaddFL{vertical grey dotted lines, }\DIFaddendFL equation~\ref{eq:equilibrium}).
        ...
        When $\beta \NbrSites \EmpiricalDeltaDeltaG$ is large, increasing $\Ne$ by an order of magnitude ({\color{YELLOW}yellow} dotted curves) \DIFdelbeginFL \DIFdelFL{very moderately impacts }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{shifts }\DIFaddendFL the equilibrium {phenotype} \DIFdelbeginFL \DIFdelFL{(}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{by a }\DIFaddendFL small \DIFdelbeginFL \DIFdelFL{$\smash{\color{YELLOW}\Delta x\eq}$)}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{amount $\Delta x\eq$}\DIFaddendFL .
        In contrast, for small $\beta \NbrSites \EmpiricalDeltaDeltaG$ ({\color{GREEN}green} curves), \DIFdelbeginFL \DIFdelFL{the equilibrium }%DIFDELCMD < {%%%
\DIFdelFL{phenotype}%DIFDELCMD < } %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$x\eq$ }\DIFaddendFL is more strongly \DIFdelbeginFL \DIFdelFL{impacted }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{shifted (large $\Delta x\eq$) }\DIFaddendFL by a change in $\Ne$\DIFdelbeginFL \DIFdelFL{(large $\smash{\color{GREEN}\Delta x\eq}$)}\DIFdelendFL .
...
In contrast, stabilizing mutations \DIFaddbegin \DIFadd{toward an optimal amino acid }\DIFaddend are rare, while \DIFdelbegin \DIFdel{destabilizing mutations }\DIFdelend \DIFaddbegin \DIFadd{on the other hand destabilizing mutations from an optimal to a non-optimal amino acid }\DIFaddend have a low probability of fixation.
...
\begin{gather}
    ...
    \Rightarrow s \simeq - \beta \EmpiricalDeltaDeltaG A y e\DIFdelbegin \DIFdel{^{\beta(\EmpiricalDeltaGmin + \EmpiricalDeltaDeltaG \NbrSites x)}}\DIFdelend \DIFaddbegin \DIFadd{^{\beta(\EmpiricalDeltaGmin + \NbrSites \EmpiricalDeltaDeltaG x)}}\DIFaddend .
\end{gather}
...
\begin{equation}
    \ln \left( \frac{1 - x\eq}{x\eq} \right) + \ln (19) = 4\Ne y A \beta \EmpiricalDeltaDeltaG \DIFdelbegin \DIFdel{\NbrSites }\DIFdelend e\DIFdelbegin \DIFdel{^{\beta(\EmpiricalDeltaGmin + \EmpiricalDeltaDeltaG \NbrSites x\eq)}}\DIFdelend \DIFaddbegin \DIFadd{^{\beta(\EmpiricalDeltaGmin + \NbrSites \EmpiricalDeltaDeltaG x\eq)}}\DIFaddend .
\end{equation}
...
\DIFaddbegin \DIFadd{Moreover, our theoretical results can be applied more broadly to protein-protein interactions using a mean-field argument (supplementary materials).
}\DIFaddend Altogether, we theoretically obtain the same linear decrease of $\omega$ with regards to either {effective population size} or expression level (in log space) under a broad variety of hypotheses.

\subsection{Simulation experiments}

\begin{figure*}[h]
    \centering
    \includegraphics[width=\textwidth, page=1] {figure-3.pdf}

    \caption[Scaling of equilibrium $\omega$ as a function of $\Ne$]
    {Scaling of equilibrium \DIFaddbeginFL \DIFaddFL{substitution rate (}\DIFaddendFL $\omega$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL as a function of \DIFdelbeginFL \DIFdelFL{log-$Ne$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{effective population size ($\Ne$) in log scale}\DIFaddendFL , under the additive {phenotype} model using Grantham distances (A,B,D) or the explicit biophysical model using a statistical potential (C), with $\NbrSites=300$ and $\beta=1.686$.
    200 replicates per $\Ne$ value are shown (dots).
    Solid lines are average over replicates, and shaded areas are 90\% \DIFdelbeginFL \DIFdelFL{confidence interval}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{inter-quantile range across the different replicates}\DIFaddendFL .
    The slope (or susceptibility ($\hat{\chi}$), is estimated by linear regression (dashed lines).
        (A): \DIFaddbeginFL \DIFaddFL{the optimal free energy of folding }\DIFaddendFL $\EmpiricalDeltaGmin$ are given in the legend, and \DIFaddbeginFL \DIFaddFL{the destabilizing effect of mutations }\DIFaddendFL $\EmpiricalDeltaDeltaG=1$.
...
\begin{figure*}[h]
    \centering
    \includegraphics[width=\textwidth, page=1] {figure-4.pdf}
    \caption[ Relaxation of $\omega$  after a change in $\Ne$]{
        Relaxation of \DIFaddbeginFL \DIFaddFL{the substitution rate (}\DIFaddendFL $\omega$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL after a change in \DIFaddbeginFL \DIFaddFL{effective population size (}\DIFaddendFL $\Ne$\DIFaddbeginFL \DIFaddFL{).
        }\DIFaddendFL Solid line corresponds to the average over $1000$ replicates and the shaded area corresponds to the $90\%$ \DIFdelbeginFL \DIFdelFL{interval }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{inter-quantile range }\DIFaddendFL among replicates.
        The mutation rate ($\mu$) is \DIFdelbeginFL \DIFdelFL{$1e{-8}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\smash{10^{-8}}$ }\DIFaddendFL per year per site, and the total evolutionary period is 700 million years.
        (A): $\beta=1.686$ for all simulations.
        The {DNA} sequence of $500$ sites is divided into exons of equal size.
        However the number of sites per exon \DIFaddbeginFL \DIFaddFL{($\NbrSites$) }\DIFaddendFL changes between simulations from $\NbrSites=5$ to $\NbrSites=500$.
        Moreover, \DIFaddbeginFL \DIFaddFL{the destabilizing effect of mutations (}\DIFaddendFL $\EmpiricalDeltaDeltaG$\DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL is changed according to the exon size such that $\NbrSites \EmpiricalDeltaDeltaG$ (and as a result, the susceptibility) are kept constant, and $\EmpiricalDeltaGmin$ is changed accordingly such that the equilibrium value $\smash{x\eq}$ is kept constant, by solving numerically equation~\ref{eq:equilibrium}.
...
We \DIFdelbegin \DIFdel{observed in our simulations }\DIFdelend \DIFaddbegin \DIFadd{thus performed simulations on which we tracked $\omega$ as a function of time after a sudden change in $\Ne$, allowing us to observe the non-equilibrium behavior, as well as the time necessary to reach the new equilibrium $\omega$.
}

\DIFadd{A characteristic observed in these non-equilibrium experiments is the discontinuity of $\omega$ after a change in $\Ne$~(figure~\ref{fig:relaxStability}A), which can be explained mechanistically.
Under low $\Ne$, the }{\DIFadd{phenotype}} \DIFadd{is far away from the optimal }{\DIFadd{phenotype}} \DIFadd{because the efficacy of selection is weaker.
A sudden increase in $\Ne$ results first in a short traction toward a more optimal }{\DIFadd{phenotype}}\DIFadd{, which results in a suddenly higher $\omega$, caused by a transient adaptation of the protein toward a higher stability.
Subsequent to the transient adaptation, under the higher $\Ne$ the }{\DIFadd{phenotype}} \DIFadd{is closer to optimal and the purification of deleterious mutations is stronger, resulting in lower $\omega$.
Conversely, the reaction to a decrease in $\Ne$ is a relaxation of the purification and thus an $\omega$ closer to the }{\DIFadd{neutral}} \DIFadd{case, which results into higher $\omega$ until reaching the new equilibrium.
To note, an increase in $\Ne$ can theoretically and possibly lead to an $\omega$ that is temporarily greater than 1 due to adaptive evolution~\mbox{%DIFAUXCMD
\citep{Jones2016}}\hspace{0pt}%DIFAUXCMD
, while a decrease in $\Ne$ always imply an $\omega < 1$, as it gives at most a }{\DIFadd{neutral}} \DIFadd{regime of relaxed selection.
}

\DIFadd{Within our simulation setup, we also observed }\DIFaddend that the determining factor of the relaxation time \DIFaddbegin \DIFadd{to reach the new equilibrium }\DIFaddend is the number of sites $\NbrSites$ (figure~\ref{fig:relaxStability}A), such that the return to equilibrium is faster for longer sequences.
...
Under low $\Ne$, the }%DIFDELCMD < {%%%
\DIFdel{phenotype}%DIFDELCMD < } %%%
\DIFdel{is far away from the optimal }%DIFDELCMD < {%%%
\DIFdel{phenotype}%DIFDELCMD < } %%%
\DIFdel{because the efficacy of selection is weaker.
A sudden increase in $\Ne$ results first in a short traction toward a more optimal }%DIFDELCMD < {%%%
\DIFdel{phenotype }%DIFDELCMD < }%%%
...
Conversely, under high $\Ne$ the }%DIFDELCMD < {%%%
\DIFdel{phenotype }%DIFDELCMD < } %%%
\DIFdel{is closer to optimal and the purification of deleterious mutationsis stronger.
The reaction to a decrease in $\Ne$ is a relaxation of the purification and thus an $\omega$ closer to the }%DIFDELCMD < {%%%
\DIFdel{neutral}%DIFDELCMD < } %%%
\DIFdel{case, which results into higher $\omega$ until reaching the point of marginal stability.
To note, an increase in $\Ne$ can theoretically and possibly lead to an $\omega$ that is temporarily greater than 1 due to adaptive evolution~\mbox{%DIFAUXCMD
\citep{Jones2016}}\hspace{0pt}%DIFAUXCMD
, while a decrease in $\Ne$ always imply an $\omega < 1$, as it gives at most a }%DIFDELCMD < {%%%
\DIFdel{neutral}%DIFDELCMD < } %%%
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{In the context of directional selection under a given genotype-phenotype-fitness map, we derive an approximation for the equilibrium phenotype at equilibrium~\mbox{%DIFAUXCMD
\citep{Charlesworth2013}}\hspace{0pt}%DIFAUXCMD
.
We relates the equilibrium phenotype to the substitution rate of selected mutations, $\omega$, which identifies in phylogenetics with the ratio of non-synonymous over synonymous substitutions (called $\dnds$).
}\DIFaddend We provide a compact analytical result for the equilibrium response (which, by analogy with thermodynamics, we call the susceptibility) of $\omega$ to changes in \DIFaddbegin \DIFadd{effective population size (}\DIFaddend $\Ne$\DIFaddbegin \DIFadd{)}\DIFaddend , and we relate this response to the parameterization of the genotype-phenotype-fitness map.
...
Furthermore, this application \DIFdelbegin \DIFdel{demonstrates }\DIFdelend \DIFaddbegin \DIFadd{illustrates }\DIFaddend that {effective population size} and protein expression level are interchangeable with respect to their impact on the response of $\omega$.
Our compact theoretical \DIFdelbegin \DIFdel{results}\DIFdelend \DIFaddbegin \DIFadd{approximations}\DIFaddend , which were obtained by making several simplifying assumptions, are supported by more complex simulations of protein evolution relaxing these assumptions.
...
\end{equation}
Averaged over \DIFdelbegin \DIFdel{all branches of the tree, the average }\DIFdelend \DIFaddbegin \DIFadd{the simulation, }\DIFaddend $\omega$ is:
\begin{align}
    \omega & = \left\langle \omega(t) \right\rangle, \\
    & = \int_{t} \omega (t) \der t,
\end{align}
where the integral is taken over \DIFdelbegin \DIFdel{all branches of the tree}\DIFdelend \DIFaddbegin \DIFadd{the complete simulation}\DIFaddend , while the integrand $\omega (t)$ is a piece-wise function changing after every point {substitution} event.

\section{Supplementary materials \& data availability}
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliographystyle{natbib}
