...
\begin{figure*}[h]
    ...
    \caption[Response of the equilibrium phenotype after a change in $\Ne$]{
        ...
        The equilibrium {phenotype} $x\eq$ is obtained when the selective pressure equals to the mutational pressure.
        The selective pressure increases exponentially with $x$ where $\NbrSites \EmpiricalDeltaDeltaG$ is the exponential growth rate ({\color{YELLOW}yellow} and {\color{GREEN}green} curves).
        When $\NbrSites \EmpiricalDeltaDeltaG$ is large, increasing $\Ne$ by an order of magnitude ({\color{YELLOW}yellow} dotted curves) very moderately impacts the equilibrium {phenotype} (small $\smash{\color{YELLOW}\Delta x\eq}$).
        In contrast, for small $\NbrSites \EmpiricalDeltaDeltaG$ ({\color{GREEN}green} curves), the equilibrium {phenotype} is more strongly impacted by a change in $\Ne$ (large $\smash{\color{GREEN}\Delta x\eq}$).
