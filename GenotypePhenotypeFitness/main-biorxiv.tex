...
\title{Substitution rate response to changes in effective population size and protein expression level}
...
Population genetics theory implies that the strength of drift, due to the stochastic sampling of mutations, is less pronounced in lineages with large {effective population size} ($\Ne$), and as a consequence, the purification by selection of weakly deleterious mutations is more effective.
...
Practically, in the case of protein-coding {DNA} sequences, and assuming that {synonymous} mutations are {neutral}, while {non-synonymous} changes affecting the amino-acid sequence are under selection, $\omega$ can be identified with the ratio of {non-synonymous} over {synonymous} {substitution} rates , called $\dnds$.
...
Such a modelling approach predicts higher {substitution} rates between amino acids with similar biochemical properties.
...
\begin{figure*}[h]
    ...
    \caption[Response of the equilibrium phenotype after a change in $\Ne$]{
        ...
        The equilibrium {phenotype} $x\eq$ is the solution of equation $\smash{\ln(\frac{1 - x\eq}{x\eq}) + \ln(19)=4\Ne \beta \DeltaDeltaG e^{\beta (\DeltaGmin + \NbrSites \DeltaDeltaG x\eq)}}$.
        ...
        When $\NbrSites \DeltaDeltaG$ is large, increasing $\Ne$ by an order of magnitude ({\color{RED}red} curves) very moderately impacts the equilibrium {phenotype} (small $\smash{\color{RED}\Delta x\eq}$).
        In contrast, for small $\NbrSites \DeltaDeltaG$ ({\color{GREEN}green} curves), the equilibrium {phenotype} is more strongly impacted by a change in $\Ne$ (large $\smash{\color{GREEN}\Delta x\eq}$).
