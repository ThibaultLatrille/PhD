...
Under neutrality each site has an independent $1/K$ chance of being the stable state and a chance $1 - 1/\Nstate$ of being unstable.
At mutational equilibrium, the distribution of phenotype $x$ is thus a binomial distribution $\mathcal{B}(\NbrSites,1-1/\Nstate)$ that has been rescaled so that its support ranges from 0 to 1 in increments of $1/\NbrSites$ rather than from 0 to $\NbrSites$ in increments of 1.

The expectation and variance of the phenotype at equilibrium can be derived as:
\begin{align}
    \E \left[ \dfrac{\mathcal{B}(\NbrSites, 1-1/\Nstate)}{\NbrSites} \right] &= 1-\dfrac{1}{\Nstate} , \\
    \Var \left[ \dfrac{\mathcal{B}(\NbrSites, 1-1/\Nstate)}{\NbrSites} \right] &= \frac{1}{\NbrSites} \dfrac{\Nstate-1}{\Nstate^2}.
\end{align}

We verified the scaling of variance at mutational equilibrium as a function of $\NbrSites$ with simulations in the case $\Nstate = 2$ and $\Nstate = 20$:
\begin{center}
    \includegraphics[width=0.6\linewidth, page=1]{mutational-variance}
\end{center}
As a result, the variance at the mutational equilibrium decreases with the number of sites ($\NbrSites = 1/\dx$).


Under a large number of sites, the binomial distribution can be approximated by a normal distribution, such that the distribution of phenotype $x$ denoted $\sigma (x)$ is:
\begin{gather}
    \sigma (x) \sim \mathcal{N}\left( 1-\frac{1}{\Nstate}, \frac{1}{\NbrSites} \dfrac{\Nstate-1}{\Nstate^2} \right). \label{eq:mut-equi}
\end{gather}
We verified the analytical approximation with simulations in the case $\Nstate = 2$ and $\Nstate = 20$:
\begin{center}
    \begin{minipage}{0.49\linewidth}
        \includegraphics[width=\linewidth]{mutational-equilibrium-K_2}
    \end{minipage}%
    \hfill
    \begin{minipage}{0.49\linewidth}
        \includegraphics[width=\linewidth]{mutational-equilibrium-K_20}
    \end{minipage}
\end{center}
...
From the mutational equilibrium distribution of phenotype $\sigma (x) $ (equation \ref{eq:mut-equi}) and the Malthusian fitness $\logfit (x)$, the mutation-selection equilibrium distribution \citep{Sella2005, Mustonen2009} is given by:
\begin{gather}
\psi (x) = \mathcal{Z} \sigma (x) \e^{4 \Ne \logfit (x)}, \\
\text{ where } \int_{0}^{1} \sigma (x) \e^{4 \logfit (x)} \der x = \mathcal{Z}^{-1}. \label{eq:mutsel-equi}
\end{gather}

An example of model developed in the manuscript is whenever fitness is equal to the folded fraction (see \ref{subsec:fitness-equal-to-folded-fraction}):
\begin{gather}
    \logfit (x) = \ln \left(  \frac{1}{1 + \e^{\beta(\DeltaGmin + \NbrSites \DeltaDeltaG x)}}\right)
\end{gather}
Using $\Ne=10^4$, $\beta=1.686$, $\DeltaGmin = \EmpiricalDeltaGmin = -118$, $\NbrSites=300$, $\DeltaDeltaG=\EmpiricalDeltaDeltaG=1$, we verified the analytical  distribution of $\psi (x)$ with simulations in the case $\Nstate = 2$ and $\Nstate = 20$:
\begin{center}
    \begin{minipage}{0.49\linewidth}
        \includegraphics[width=\linewidth]{selection-equilibrium-K_2}
    \end{minipage}%
    \hfill
    \begin{minipage}{0.49\linewidth}
        \includegraphics[width=\linewidth]{selection-equilibrium-K_20}
    \end{minipage}
\end{center}

We could not find a tractable analytical solution for the expectation $\E \left[ \psi (x) \right]$ and variance $\Var \left[ \psi (x) \right]$.
However, simulations (shown below) provides the insight that variance at mutation-selection equilibrium is lower than variance at mutational equilibrium: $\Var \left[ \psi (x) \right] \leq \Var \left[ \sigma (x) \right]$.
\begin{center}
