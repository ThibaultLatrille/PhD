...
\newcommand{\freq}{\psi}
\newcommand{\Freq}{\UniDimArray{\freq}}
% \acrshort{DNA}
...
\newcommand{\sumSetAa}{_{\aminoacid=1}^{20}}
...
\newcommand{\node}{\text{n}}
\newcommand{\taxon}{\node}
\newcommand{\Settaxon}{\taxon \in \{1, \hdots, \Ntaxa \}}
\newcommand{\Ntaxa}{P}
\newcommand{\treeroot}{0}
...
\newcommand{\traitj}{b}
\newcommand{\trait}{X}
\newcommand{\Trait}{\UniDimArray{\trait}}
\newcommand{\Ntrait}{L}
...
However, computational models based on biophysics of protein stability have suggested that $\omega$ can also be independent of $\Ne$.
...
Assuming a gamma distribution for the {DFE}, a key result obtained in this context is an approximate scaling of $\omega$ as a function of $\Ne$ (i.e.~$\omega \sim \Ne^{-k}$), where $k$ is the shape parameter of the {DFE}.
In practice, DFEs are strongly leptokurtic, which thus predicts a weak negative relation between $\omega$ and $\Ne$.

However, the DFE may not be fixed a priori, instead it may naturally emerge from an underlying fitness landscape and thus vary depending on the position of the population in the fitness landscape~\citep{Lourenco2013}.
In an alternative approach, based on genotype-fitness maps modelling the underlying fitness landscape, the selective effect of a mutation depends on the fitness of both the source and the target amino acids involved in the mutation event~\citep{Halpern1998, Rodrigue2010, Tamuri2012}.
...
Conversely, one striking theoretical result was that $\omega$ is in fact predicted to be independent of $\Ne$, whenever (i) the fitness is a log-concave function of a {phenotype} and (ii) the {phenotype} itself is equimutable.
...
This general theoretical argument was obtained using a heuristic derivation in the deterministic limit, that is, ignoring the fluctuations at stationarity.
It has been invoked in the context of \textit{in silico} experiments of protein sequence evolution, assuming that proteins are under selection for their thermodynamic stability, with fitness being proportional to the folding probability of the protein~\citep{Goldstein2013}.
...
To this effect, we develop a general mathematical approach assuming a given genotype-phenotype-fitness map.
We determine an approximate solution to the equilibrium phenotype, as in \citet{Charlesworth2013}, which is then related to the substitution rate $\omega$.
Subsequently, we derive a quantitative approximation of the response of $\omega$ to changes in $\Ne$ and expression level, as depicted in figure~\ref{fig:Summary}.
We apply our mathematical approach to fitness models based on protein thermodynamics.
...
Analytically, the fitness function is given by the Fermi Dirac distribution~\citep{Goldstein2011} and is typically close to $1$~\citep{Serohijos2014}, leading to a first-order approximation:
\begin{gather}
...
Altogether, any mutation in the sequence can then have a phenotypic effect of $0$ or $\dx=1 /\NbrSites$, with probabilities of transitions from $x$ to $x'$ equal to:
\begin{gather}
    \begin{cases}
        \proba ( x' = x + \dx ) = 1-x, \\
...
Under the assumption that fluctuations around the equilibrium value are small (in supplementary materials), the equilibrium distribution of trait can be approximated by an equilibrium trait value, thus using a deterministic approximation, as in \citet{Cherry1998}.
This equilibrium value can be determined by expressing the constraint that the selection coefficient of {substitutions} is expected to be null on average~\citep{Goldstein2013}.
...
\begin{figure*}[h]
    ...
    \caption[Response of the equilibrium phenotype after a change ineffective population size]{
        Response of the equilibrium {phenotype} ($x\eq$) after a change in effective population size~($\Ne$).
        The equilibrium {phenotype} $x\eq$ is obtained when the selective pressure equals to the mutational pressure (vertical grey dotted lines, equation~\ref{eq:equilibrium}).
        ...
        When $\beta \NbrSites \EmpiricalDeltaDeltaG$ is large, increasing $\Ne$ by an order of magnitude ({\color{YELLOW}yellow} dotted curves) shifts the equilibrium {phenotype} by a small amount $\Delta x\eq$.
        In contrast, for small $\beta \NbrSites \EmpiricalDeltaDeltaG$ ({\color{GREEN}green} curves), $x\eq$ is more strongly shifted (large $\Delta x\eq$) by a change in $\Ne$.
...
Moreover, only the compound parameter $\beta \NbrSites \EmpiricalDeltaDeltaG$ has an impact on the slope of the linear relationship.
...
\begin{gather}
    ...
    \Rightarrow s \simeq - \beta \EmpiricalDeltaDeltaG A y e^{\beta(\EmpiricalDeltaGmin + \NbrSites \EmpiricalDeltaDeltaG x)}.
\end{gather}
...
\begin{equation}
    \ln \left( \frac{1 - x\eq}{x\eq} \right) + \ln (19) = 4\Ne y A \beta \EmpiricalDeltaDeltaG e^{\beta(\EmpiricalDeltaGmin + \NbrSites \EmpiricalDeltaDeltaG x\eq)}.
\end{equation}
...
Moreover, our theoretical results can be applied more broadly to protein-protein interactions using a mean-field argument (supplementary materials).
...
\begin{figure*}[h]
    ...
    {Scaling of equilibrium substitution rate ($\omega$) as a function of effective population size ($\Ne$) in log scale, under the additive {phenotype} model using Grantham distances (A,B,D) or the explicit biophysical model using a statistical potential (C), with $\NbrSites=300$ and $\beta=1.686$.
    200 replicates per $\Ne$ value are shown (dots).
    Solid lines are average over replicates, and shaded areas are 90\% inter-quantile range across the different replicates.
    The slope (or susceptibility ($\hat{\chi}$), is estimated by linear regression (dashed lines).
        (A): the optimal free energy of folding $\EmpiricalDeltaGmin$ are given in the legend, and the destabilizing effect of mutations $\EmpiricalDeltaDeltaG=1$.
...
These simulation experiments illustrate, first, that the relation between $\omega$ and log-$\Ne$ is indeed linear, at least in the range explored here, and that the slope of the linear regression matches the expected theoretical value (figure~\ref{fig:GoldsteinVsToy}A).
...
\begin{figure*}[h]
    ...
    \caption[ Relaxation of $\omega$  after a change in $\Ne$]{
        Relaxation of the substitution rate ($\omega$) after a change in effective population size ($\Ne$).
        Solid line corresponds to the average over $1000$ replicates and the shaded area corresponds to the $90\%$ inter-quantile range among replicates.
        The mutation rate ($\mu$) is $\smash{10^{-8}}$ per year per site, and the total evolutionary period is 700 million years.
        (A): $\beta=1.686$ for all simulations.
        The {DNA} sequence of $500$ sites is divided into exons of equal size.
        However the number of sites per exon ($\NbrSites$) changes between simulations from $\NbrSites=5$ to $\NbrSites=500$.
        Moreover, the destabilizing effect of mutations ($\EmpiricalDeltaDeltaG$) is changed according to the exon size such that $\NbrSites \EmpiricalDeltaDeltaG$ (and as a result, the susceptibility) are kept constant, and $\EmpiricalDeltaGmin$ is changed accordingly such that the equilibrium value $\smash{x\eq}$ is kept constant, by solving numerically equation~\ref{eq:equilibrium}.
...
We thus performed simulations on which we tracked $\omega$ as a function of time after a sudden change in $\Ne$, allowing us to observe the non-equilibrium behavior, as well as the time necessary to reach the new equilibrium $\omega$.

A characteristic observed in these non-equilibrium experiments is the discontinuity of $\omega$ after a change in $\Ne$~(figure~\ref{fig:relaxStability}A), which can be explained mechanistically.
...
Subsequent to the transient adaptation, under the higher $\Ne$ the {phenotype} is closer to optimal and the purification of deleterious mutations is stronger, resulting in lower $\omega$.
Conversely, the reaction to a decrease in $\Ne$ is a relaxation of the purification and thus an $\omega$ closer to the {neutral} case, which results into higher $\omega$ until reaching the new equilibrium.
...
Within our simulation setup, we also observed that the determining factor of the relaxation time to reach the new equilibrium is the number of sites $\NbrSites$ (figure~\ref{fig:relaxStability}A), such that the return to equilibrium is faster for longer sequences.
...
In the context of directional selection under a given genotype-phenotype-fitness map, we derive an approximation for the equilibrium phenotype at equilibrium~\citep{Charlesworth2013}.
...
We provide a compact analytical result for the equilibrium response (which, by analogy with thermodynamics, we call the susceptibility) of $\omega$ to changes in effective population size ($\Ne$), and we relate this response to the parameterization of the genotype-phenotype-fitness map.
...
Furthermore, this application illustrates that {effective population size} and protein expression level are interchangeable with respect to their impact on the response of $\omega$.
Our compact theoretical approximations, which were obtained by making several simplifying assumptions, are supported by more complex simulations of protein evolution relaxing these assumptions.
...
\end{equation}
Averaged over the simulation, $\omega$ is:
\begin{align}
