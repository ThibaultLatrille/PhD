...
\begin{align}
    ...
    \Rightarrow \logfit (x) \simeq e^{\beta (\DeltaGmin + \NbrSites \DeltaDeltaG x)},
\end{align}
where $\wrightfit$ is the Wrightian fitness for a given phenotype and $\logfit = \ln ( \wrightfit )$ is the Malthusian fitness.
...
\begin{equation}
    s \left( \Seqi,\Seqj\right) = \sum\limits_{1 \leq \site \leq \NbrSites} \ln \left( \dfrac{G_{\site} \left(\Seqj(\site) \right)}{G_{\site} \left(\Seqi(\site) \right)} \right) ,
\end{equation}
