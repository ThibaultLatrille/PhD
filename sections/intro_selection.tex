...
Practically, in so-called site-specific phylogenetic \gls{codon} models, $\dnds$ is allowed to vary across sites, either via a finite mixture~\citep{Yang2001}, an infinite mixture~\citep{Huelsenbeck2006}, or as random effects from a parametric distribution~\citep{Lartillot2013}.
...
In contrast, mechanistic phylogenetic codon model acknowledge that specific amino-acids physico-chemical properties is a crucial ingredient to represent evolution of protein coding \acrshort{DNA} sequences.
...
But assumptions of the models can be broken, resulting in discrepancy between $\omega_0$ and $\omega$, modeled as multipliying factor $\omega_0$ \cite{Rodrigue2016}:
\begin{equation}
