...
Throughout this development, we will be assuming that the tree topology ($\tau$) is considered know and fixed.
...
As an example for better readability, a simple illustrative tree given in figure \ref{fig:tree} will be used prior to giving to general formulas.
\begin{figure}[H]
	\begin{center}
		...
		\path[-]
		(1) edge [] node [above right] {$\branchlength^{(1)}$} (0)
		(2) edge [] node [below right] {$\branchlength^{(2)}$} (0)
		(5) edge [] node [above] {$\branchlength^{(5)}$} (0)
		(5) edge [] node [above left] {$\branchlength^{(3)}$} (3)
		(5) edge [] node [below left] {$\branchlength^{(4)}$} (4);
		\end{tikzpicture}
	\end{center}
	\caption[Illustrative phylogenetic tree]{Illustrative phylogenetic tree. Internal nodes ($\state_0$ and $\state_5$) are represented in yellow, while extant nodes ($\state_1$ to $\state_4$) for which the state is know from the observed data is represented in blue. The node $0$ is considered the root of the tree, although defining is not strictly required since the process is reversible.}
	\label{fig:tree}%
\end{figure}

In the example of the illustrated tree, from the observed data for the extant nodes $\Data\siteexp = \{ \state_1,\state_2, \state_3, \state_4 \}$, at site $\site$, the likelihood given that the states of the internal nodes are known is computed as:
\begin{align}
...
And because the process is reversible, the equilibrium frequencies of codons satisfies the equations:
\begin{align}
...
\begin{equation}
p(\Submatrix|\data) \propto p(\data|\Submatrix)p(\Submatrix)
\end{equation}
Simply stating that the \gls{posterior} is proportional to the \gls{likelihood} time the \gls{prior}.

Because we have seen how to compute the likelihood in the previous section, the difficulties now is shifted to the definition of the prior distribution, where these difficulties come in two flavors.
Firstly, the parameters of evolution for which we have prior distribution are the parameters of interest such as the selection coefficient, or on the mutation rate, but we do not have a prior distribution on the substitution matrix as a whole.
...
\begin{figure}[H]
	...
	\node[state] (sb) [RED, right=of BL] {$\StickBreaking$};
	\node[state] (Ne) [BLUE, above right=of Q] {$\bm{F}\siteexp$};
	\node[state] (sb) [RED, right=of BL] {$\StickBreaking$};
	...
	(BL) edge [BLUE] node [above] {} (sb)
	
	\end{tikzpicture}
	\caption{Directed acyclic graph of dependencies between variables}{Nodes of the directed acyclic graph are the variables, and edges are the functions. Hyper-parameters are depicted in {\color{RED}{red}} circle, random variables in {\color{BLUE}{blue}} circles, and transformed variables in black. Dotted {\color{BLUE}{blue}} line denotes a drawing from a random distribution, and black solid lines denote a function. $\theta$ }
	\label{fig:graph}%
\end{figure}

Secondly, once realizing that the prior distribution can be boiled to a set of simpler prior distributions, the difficulty arise from the high dimensionality of the parameter space, known as the curse of dimensionality.
