...
At one particular site, under a static fitness landscape, the substitution rate between codons are given by the product of the mutation rate and the probability of fixation, given by the formula of Kimura (1983) \citet{kimura_neutral_1983}:
\begin{equation}
\label{codonSubRates}
Q_{x, y}^{(k)} =
\begin{dcases}
...
\label{subFlowSyn}
\end{eqnarray}
Following Spielman and Wilke \citet{Spielman2015a}, the rate of non-synonymous to synonymous substitution is thus :
\begin{eqnarray}
\omega^{(k)}(\lambda, F^{(k)})
...
\begin{figure}[thbp]
	...
	\end{center}
	\caption{Parameters of the mutation-selection model}{One parameter for mutational bias, and one for strength of selection.}
\end{figure}
...
\begin{figure}[thbp]
	...
	\end{center}
	\caption{Diversity of amino-acids}{Sequence-diversity is higher than site-diversity. Diversity decreases with mutational bias.	Site-diversity decreases with selection. Sequence-diversity increase with selection.}
\end{figure}
...
\begin{figure}[thbp]
	...
	\end{center}
	\caption{AT/GC composition of the alignment}{Observed AT/GC at the third codon position matches the mutational bias. Selection is balancing the mutational bias.}
\end{figure}
...
\begin{figure}[thbp]
	...
	\end{center}
	\caption{$\omega$ as a function of the parameters.}{Non-synonymous fixation bias is always lower than. Non-synonymous fixation bias decrease with the strength of selection. Non-synonymous fixation bias is unaffected by mutational bias.}
\end{figure}
...
\begin{figure}[thbp]
	...
	\end{center}
	\caption{Fixation bias of non-synonymous mutations}{Mutation biased from GC to AT leads to a fixation bias in the opposite direction. More generally, mutation bias leads to balancing fixation bias. This is confounding factor with gBGC.}
\end{figure}
...
\begin{figure}[thbp]
	...
	\end{center}
	\caption{Estimation of mutation and fixation bias}{Modeling a single fixation bias leads to skewed estimation of mutation bias. Modeling multiple fixation bias leads to precise estimation of mutation bias. Estimation of fixation bias doesn't depend on the underlying mutation bias.}
\end{figure}
