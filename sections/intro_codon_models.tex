...
Subsequently, mechanistic codon models are presented, assuming that the \acrshort{DNA} sequence is at mutation-selection balance under a fixed fitness landscape over the $20$ amino acids.
...
\begin{equation}
    ...
    \end{pmatrix}
\end{equation}
Most often, this matrix is assumed to be a general time-reversible (\acrshort{GTR}) and thus defined by nucleotide equilibrium frequencies ($\bm{\mutequi}$) and by symmetric exchangeability rates ($\bm{\exchan}$):
\begin{equation}
...
Thus, if codons $\ci$ to $\cj$ are only a mutation away, let $\nucitoj$ denote the nucleotide change between them (\textit{e.g.} $\nuc(AAT, AAG) = TG$).
...
Under the Muse \& Gaut formalism, the codon equilibrium frequencies ($\bm{\pi}$ depend only on the equilibrium nucleotide frequencies ($\bm{\mutequi}$), but not on $\omega$:
\begin{align}
...
One particularly important application of classical codon models has been to characterize genes under positive selection (\textit{i.e.} with a $\dnds > 1$), or sites within genes or specific lineage under accelerated evolution.
...
Indeed, recurrent positive selection might often be concentrated in a small region of the protein (\textit{e.g.} domain or site of the protein that is more directly interacting with a pathogen), the rest of the protein being under a regime of purifying selection.
...
The \citet{Halpern1998} formalism assumes that the protein-coding sequence is at mutation-selection balance under a fixed fitness landscape, with a fitness that is multiplicative across sites (\textit{i.e.} without epistasis).
...
\end{align}
The stationary frequency of a codon is ultimately the product of the nucleotide frequencies at its three positions ($\bm{\mutequi}$) and the scaled Wrightian fitness of the amino-acid ($\e^{\Fiti}$).
...
In particular, selective pressures may change following one (or more) transitions to a new environment (\textit{e.g.}: a new host).
...
\begin{table}[H]
    ...
    \begin{tabu}{|c|l|}
        ...
        \multirow{2}{*}{$\omega_*$} & Scaling factor for all non-synonymous substitutions in the \citet{Halpern1998} formalism. \\
...
The Halpern \& Bruno formalism assumes that the fitness landscape is not dependent on time.
Alternatively, fitness landscapes changing with time are known as seascape~\citep{Mustonen2009}.
Because of the movement of the fitness landscape, similarly to Red-Queen dynamics, the current is more likely to slide into a fitness valley rather than on top of a peak when the landscape is moving.
...
The resulting dynamics is that selection pushes the sequence to climb up the landscape constantly, and the protein sequence is tracking a constantly moving fitness optimum.
