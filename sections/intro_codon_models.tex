...
As a result, throughout this manuscript the notation $\omega$ will be used specifically and uniquely for the multiplicative factor in the \citet{Muse1994} formalism (see~\ref{subsec:MG-formalism}).
Alternatively, the term $\dnds$ will be used generically for describing method estimating the ratio of non-synonymous over synonymous substitution rates, without accounting for the underlying variability of formalism.
...
Contrarily, whenever $\omega$ is used, it refers to the specific Muse \& Gaut formalism of section~\ref{subsec:MG-formalism}, recruited for its mechanistic formalism.
...
More precisely, variants of these models have been developed that can provide estimates of $\dnds$ for the whole gene, for each site within a gene, or for each branch within a phylogenetic tree.
One particularly important application has been to characterize genes under positive selection (i.e. with a $\dnds > 1$), or sites within genes or specific lineage under accelerated evolution.
However, these codon models have also proved to be valuable to quantify and asses the modulation of the selective constraint more generally imposed on protein coding sequences (see \ref{sec:classical-codon-biophysics}).
...
In bayesian inference, sites under adaptive evolution are then detected based on their empirical Bayes posterior probability that $\dnds > 1$~\citep{Yang2005}.
...
Thereafter, integrative inference methods combining both molecular sequences and quantitative traits have been developed, jointly modeling the variation of all of these variables using a single multivariate Brownian process.
...
However, the universality and robustness of the correlation between $\dnds$ and life-history traits is still debated, and further inverstigation are required~\citep{Nabholz2013,Lanfear2014,Figuet2016, Bolivar2019}.

\subsection{Variation across sites and branches}
\label{subsec:variation-across-sites-and-branches}

Naturally, both these space (site-specific) and time (branch-specific) refinements mentioned above led to the development of so called branch-site models~\citep{Yang2002a, Zhang2004, Pond2011}.
...
Conversely, mechanistic codon models seek to predict individually all substitution rates, for each position and between each pair of codons, in an explicit model of the adaptive landscape.
...
\end{align}
Where the stationary frequency of codon is ultimately the product of the nucleotide frequencies at three positions ($\bm{\mutequi}$) and the scaled Wrightian fitness of the amino-acid ($\e^{\Fiti}$), which is sometimes called propensities of amino-acid.
...
\end{align}
Such that scaled selection coefficients are related to the stationary codon frequencies of the process:
\begin{equation}
...
\end{align}
As a result, the substitution rate from codon $\ci$ to $\cj$ can be approximated as a plugin estimator from both the mutionnal process and the amino-acid frequencies, independently estimated.
...
Comparison of both inference approaches yield similar results in terms of estimated profiles and their induced selective constraint on protein coding DNA sequences~\citep{Spielman2016a}.
Finally, instead of estimating the fitness landscape directly on the multiple sequence alignment, deep mutational scanning approaches can be used to estimate fitness profiles experimentally~\citep{Bloom2014,Bloom2014a}, presented in chapter~\ref{chap:intro-physic-proteins}.

\subsection{Modulating the fitness landscape across branches}
\label{subsec:modulating-the-fitness-landscape-across-branches}

In the context of mutation-selection applied to amino-acid sequence, observing that two subdivision of the tree share different amino-acid profile can be interpreted as directional selection and related to changes in environment~\citep{Tamuri2009}.
...
In this context, if a specific codon site is responsible for the phenotypic convergence, the species sharing the convergent phenotype should also share convergence of amino-acid profiles at this specific site \citep{Parto2018a,Parto2018a}

\subsection{Mutation-selection and codon usage}
\label{subsec:model-codon-usage}

Another example of mutation-selection mechanistic codon model is on modelling codon usage bias, where each synonymous codon of the same amino-acids have different fitness~\citep{Yang2008}.
Such hybrid model between classical and mutation-selection formalism have the interest of measuring the average effect of selection on non-synonymous through $\dnds$ without making the assumption that synonymous mutations are neutral.
...
Using equation~\ref{eq:relative-sub-rate} on the subset of non-synonymous mutations thus gives the mean scaled fixation probability over non-synonymous mutations ($\nu_{\text{MG}}$) at stationarity:
\begin{align}
...
In this vain, $\dnds$ can alternatively be parameterized as function of the average scaled selection coefficient~\citep{Nielsen2003}:
\begin{align}
    \dnds & = \dfrac{S}{1 - \e^{-S}}
\end{align}
...
Likewise, using equation~\ref{eq:relative-sub-rate} on the subset of non-synonymous mutations gives the mean scaled fixation probability over non-synonymous mutations ($\nu_{\text{HB}}$) at stationarity:
\begin{align}
...
This deviation can be captured as a multiplying factor $\omega_*$~\citep{Rodrigue2016}:
\begin{equation}
...
\begin{table}[H]
    ...
    \begin{tabu}{|c|l|}
        ...
        $\nu$ & Mean scaled fixation probability of non-synonymous mutations. \\ \hline
        \multirow{2}{*}{$\omega$} & Scaling factor for all non-synonymous substitutions in \citet{Muse1994} formalism. \\
        & Also, $\nu_{\text{MG}}=\omega$ in Muse \& Gaut formalism. \\ \hline
        $\omega_0$ & Induced $\nu_{\text{HB}}$ in \citet{Halpern1998} mechanistic formalism. \\ \hline
        \multirow{2}{*}{$\omega_*$} & Scaling factor for all non-synonymous substitutions in \citet{Halpern1998} formalism. \\
        & Hence $\nu = \omega = \omega_* \omega_0 $ \\ \hline
    \end{tabu}}
...
A departure from near-neutrality with a $\omega \geq \omega_0$ is a signature of an ongoing Red-Queen process and that the protein is under ever changing adaptation.
