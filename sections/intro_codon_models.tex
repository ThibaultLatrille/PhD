...
More precisely, molecular evolution requires either a given selection coefficient associated to mutation, or that the fitness of each particular sequence be defined.
...
To this aim, this chapter will first present the genetic code and classical phylogenetic \glspl{codon} models, which can quantify the strength of selection acting on proteins trough an aggregate parameter (called $\omega$ or $\dnds$).
...
Finally the relationship between classical and mechanistic models is investigated, and the interpretation of the discrepancy between both models is analyzed.
...
Just as \acrshort{DNA} is oriented because of the assymetry of nucleotides, proteins are oriented due to the assymetry of amino acids. One end is called the \gls{N-ter}, and the other end, the C-terminus, and each amino acid will interact with the other amino acids in its spatial vicinity.
...
Moreover, three stop \glspl{codon} signal the termination of the protein, such that 61 of the 64 \glspl{codon} are used to encode amino acids.
Since there are 61 coding \glspl{codon} and only 20 amino acids, there is a necessary redundancy in the code.
Thus, amino acids are encoded by synonymous \glspl{codon}, which are interchangeable in the sense of producing the same amino acid, with the notable exception of methionine and tryptophan, which are only encoded by a singe codon.
...
\begin{table}[H]
    ...
    The synonymous \glspl{codon} encoding for the same amino-acid are usually different in their third \gls{codon} position, the wooble base.
    }
...
On the other end, \acrshort{tRNA} complexes bind uniquely with one the $20$ amino acid, where the catalytic reaction is performed by aminoacyl-tRNA synthetase~\citep{Rich1976}.
...
This subset of anticodon sequences necessary to bind all $61$ \glspl{codon} is due to non canonical base pairing\footnote{Canonical base pairing are A-U and G-C, where thymin (T) is replaced by uracil (U) in RNA}.
...
If the anticodon contains a guanine at first position, \glspl{codon} with either U or C at the third position can bound to this anticodon, and this phenomenon explains why there is not any non-synonymous {transition} from only U to C at the third position, and why synonymous \glspl{codon} usually end with T or C.
Also, if the anticodon contains an inosine at the first position, \glspl{codon} with either C, U or A at the third position can bound to this anticodon, such that for example leucine encoded by three \gls{codon} (AUU, AUC, AUA) can be bound by the unique anticodon IAU.
...
For genes that are expressed at high levels, the \gls{codon} used is biased in favor of the \glspl{codon} that have an high \acrshort{tRNA} concentration in the cell, ultimately increasing the expression rate and decreasing the rate of mistranslation by reducing the time of occupancy of an open site.
...
However in the scope of this manuscript, such selection between synonymous \glspl{codon} will not be considered.
...
\begin{table}[H]
    ...
    Number of possible one nucleotide non-synonymous {transitions} between amino acids, integrating over the underlying \glspl{codon}, represented as an adjacency matrix..
...
Moreover, it is possible that some pairs of amino acids are not accessible through a single non-synonymous {transition} between the underlying \glspl{codon}.
...
\begin{figure}[htbp!]
    ...
    Nodes correspond to \glspl{codon} (left panel) and amino acids (right panel), and their color represents the encoded amino-acid.
    ...
    Similarly, an edge between two amino acids correspond to a one nucleotide non-synonymous {transition} between the underlying \glspl{codon}, and the width of the edges represents the number of such possible {transitions}.
    Non-synonymous {transition} are represented in a color gradient, while synonymous {transitions} are depicted in black.
    ...
    Codons encoding for the same amino-acid are all fully connected by synonymous changes, expect for serine where a {transition} from the set {TCT, TCG, TCC, TCA} to the set {AGT, AGC} requires passing through another amino-acid, hence at least two non-synonymous {transitions}.
...
With the definition given above, $\omega$ identifies with the ratio of the rate of \glspl{non-synonymous} over the rate of \glspl{synonymous}, hence $\dnds$.
...
Moreover, these codon models have also proved to be valuable to quantify and asses the modulation of the selective constraints more generally imposed on protein coding sequences (see \ref{sec:classical-codon-biophysics}).
...
Sites under adaptive evolution are then detected based on their empirical Bayes posterior probability, such that $\dnds > 1$~\citep{Huelsenbeck2004,Yang2005}.
...
A long series of site models has been proposed, most of which have been implemented in PAML~\citep{Yang1997a,Yang2007}, but also in MrBayes for the infinite mixture version~\citep{Huelsenbeck2001, Ronquist2012}.
...
Alternatively, $\dnds$ can be modeled as a continuous trait, varying continuously along the phylogeny, and susceptible to show phylogenetic inertia.
To account for this, $\dnds$ is not mathematically formalized as a parameter anymore, but instead, it is modeled as a stochastic process, and more specifically, a log-Brownian process, splitting at each node of the tree into independent processes.
...
The external factors determining the variation $\dnds$ across lineages has subsequently been investigated, primarily focused on environmental variables and life-history traits that can vary between species.
...
Once fitted to the data, the classical Muse \& Gaut (MG) formalism return estimates of mutation rates and $\omega$ (see subsection~\ref{subsec:MG-formalism}).
...
Such equation is also true for any classical codon model formalism, where this identity between $\avgpfix$ and $\dnds$ bears many importance.
...
On the other hand, the mechanistic codon models in the Halpern \& Bruno (HB) formalism return estimates of mutation rates and fitness profiles of amino acids (see subsection~\ref{subsec:HB-formalism}).
...
Altogether, classical \gls{codon} \gls{substitution} model will interpret a mechanistic mutation-selection model as purifying selection ($\omega < 1$).
...
\begin{table}[H]
    ...
    \begin{tabu}{|c|l|}
        ...
        \multirow{2}{*}{$\omega$} & Scaling factor for all non-synonymous substitutions in \citet{Muse1994} formalism. \\
        & Also, $\avgpfix_{\text{MG}}=\omega$ in Muse \& Gaut formalism. \\ \hline
        $\omega_0$ & Induced $\avgpfix_{\text{HB}}$ in \citet{Halpern1998} mechanistic formalism. \\ \hline
        \multirow{2}{*}{$\omega_*$} & Scaling factor for all non-synonymous substitutions in \citet{Halpern1998} formalism. \\
        & Hence $\avgpfix = \omega = \omega_* \omega_0 $ \\ \hline
    \end{tabu}}
