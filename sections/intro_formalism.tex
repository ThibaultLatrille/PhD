...
However, as the size of the population approaches infinity (\textit{i.e.} $ N \to \infty$), and assuming that the scaled selection pressure ($N s $) remains constant, the discrete Markov process given above can be closely approximated by a continuous-time and continuous-space diffusion process.\\
...
Starting from an initial frequency, the Wright-Fisher process eventually reaches absorption: the derived allele either invades the population or dies out.
Under the continuous-time and continuous-space diffusion approximation, a partial differential equation known as the Kolmogorov backward equation can be used to derive the probability of these two alternative events.
\citet{Kimura1962} derived the probability of fixation ($P_{\mathrm{fix}}$) of a derived allele with selection coefficient $s$ and initial frequency $p$ if the selection coefficient is small ($|s| \ll 1$):
\begin{align}
...
\begin{figure}[h]
    ...
    The selection coefficient of the derived allele is scaled by population size $S=4 \Ne s$.
...
Because it is a single mutant, the initial frequency of the derived allele is $p = 1 / 2 \Ne$, and the probability of fixation ($P_{\mathrm{fix}}$) is given by:
\begin{align}
...
\end{align}
Altogether, the fixation probability of a selected allele relative to a neutral allele is solely dependent on the scaled selection coefficient:
\begin{align}
...
More precisely, $g(x) \der x $ is the expected time for which the population frequency of derived allele is in the range $(x, x+\der x)$ before eventual absorption, and can be derived using the Kolmogorov forward equation:
\begin{align}
...
At each generation, the expectation for the number of possible mutants is $2\Ne u$, and each of these mutants has a probability $P_{\mathrm{fix}}$ to result in a substitution.
Altogether, the instantaneous rate of substitution from allele $A$ to $B$, denoted $Q_{A \to B}$, is equal to the rate of mutation ($\mu_{A \to B}$) multiplied by the probability of fixation of the mutation $P_{\mathrm{fix}}(s_{A \to B}, \Ne)$ and scaled by the number of possible mutants at each generation ($2\Ne$):
\begin{align}
...
As a consequence, the substitution rate $Q_{A \to B}$ simplifies to:
\begin{align}
...
Moreover, under the assumption of a static fitness landscape, a mutation-selection point substitution process at equilibrium results in $\avgpfix \leq 1$ \footnote{However, I haven't found a proof for this claim in the bibliography.}.
...
I argue that scientific research endeavour is also an exploration-exploitation dilemma, which is arguably externally pressured to pursue exploitation, through funding of impactful research and a \textit{publish-or-perish} systemic culture in the early career stage.
...
This explains why sex is favoured in fluctuating environments.
