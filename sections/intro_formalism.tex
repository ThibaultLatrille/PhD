...
Such effort to infer mechanisms backward in time requires a theoretical model that generate diversity forward in time, where we can play this model forward and derive how likely such model would generate the pattern of sequence distribution we observe.
To do so, however, first requires a theoretical model of the generating process forward in time.
...
Working out the long-term molecular evolutionary process first requires to formalize what happens in the short time period within populations.
...
One quantity of interest is the mean \gls{substitution} rate to the mean mutation rate, called $ \nu $:
\begin{align}
...
\begin{table}[H]
    ...
    \begin{tabu}{|l|c|c|c|}
        ...
        \hline Equilibrium \gls{substitution} rate & \multirow{2}{*}{ $\nu$} & \multirow{2}{*}{$[0, 1]$ for purifying selection} \\
        over mutation rate & & \\
        \hline
    \end{tabu}}
