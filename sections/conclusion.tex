...
Our framework was tested against simulated data and the applied to empirical data in mammals, isopods, primates and drosophila.
...
The second manuscript leveraged the trails of substitutions along the lineages to estimate variation in $\Ne$, under the premise that lineages with large $\Ne$ are expected to undergo stronger purifying selection, thus a lower mean probability of fixation ($\nu$).
...
As a result, all of these correlation between mean fixation probability ($\nu$), effective population size ($\Ne$) and expression level should be articulated together and quantified in relation to the structure of the fitness landscape.
...
In this context, other aspects of protein biophysics could be explored such as protein-protein interactions, which could lead to a stronger response of the mean fixation probability to changes in $\Ne$.
...
This effect first the low response of the mean fixation probability to changes in $\Ne$ in chapter~\ref{chap:GenoPhenoFit}.
Indeed, in that case, all sites of the sequences are involved in non-specific epistasis.
TODO Link between phenomenological and mechanistic approach.
Moreover, this explains the low magnitude of $\Ne$ variation estimated with site-independent mechanistic codon models in chapter~\ref{chap:MutSelDrift}.
Empirically, it appears that the susceptibility of $\nu$ to changes in $\Ne$ is between these two extremes, namely that of site independent fitness landscapes, and the other extreme of a single univariate phenotype controlled by all sites.
...
TODO
It would need to be part of a larger discussion of what is known about the role of epistasis in molecular evolution of proteins and lead to further or deeper reflection on the implications.
This is a conceptual point that, to your knowledge, had never really identified until now.
A small inventory of other implications already known are interlocking or Stokes shift, downward impact on the $\dnds$ predicted by the mutation-selection models.

TO DO
The very long slow dynamic time constants you are talking about are of the order of the depth of phylogeny.
And so, in the absence of epistasis, you shouldn't even see LHT or $\dnds$ correlations because of it.
Conversely, the fact that we see it is in itself an important indication of the presence of epistasis.

TO DO
We should first give some discussion points in these directions (or other than you think), and in an order to be found.
...
In particular, if the sequences are under recurrent positive selection, the mean fixation probability ($\nu$) of non-synonymous mutations will tend to be higher than predicted by the purely nearly-neutral model.
...
Once such models have been fitted to empirical data, the estimated parameters can then be confronted with independent empirical estimates, which allow to robustly test the model since orthogonal estimation of biological and ecological parameters should be congruent~\citep{Dasmeh2014}.
However they are computationally very intensive.
As a result, they tend to make a number of strong simplifying assumptions (such as no epistasis), which can have detrimental effects on the robustness of the inference (see chapter ~\ref{chap:MutSelDrift}).
