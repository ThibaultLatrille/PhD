...
def codon_circos(cmap='tab20', filetype="pdf"):
    cm = plt.cm.get_cmap(cmap)
    cmappable = ScalarMappable(norm=Normalize(vmin=0, vmax=20), cmap=cm)

    g_codons = Graph(directed=False)
    ...
    state = NestedBlockState(g_codons, bs=[g_codons.vp.aa_index, g_codons.vp.codon_index], sampling=False)
    ...
    graph_draw(g_codons, pos=pos, edge_control_points=cts, edge_gradient=g_codons.ep.grad, edge_pen_width=2.5,
...
def amino_acid_circos(cmap='tab20', filetype="pdf"):
    cm = plt.cm.get_cmap(cmap)
    cmappable = ScalarMappable(norm=Normalize(vmin=0, vmax=20), cmap=cm)

    g_aa = Graph(directed=False)
    ...
    graph_draw(g_aa, pos=pos, edge_control_points=cts, vertex_anchor=0, vertex_text=g_aa.vp.aa,
...
for cmap in ['tab20', 'tab20b', 'tab20c']:
    amino_acid_circos(cmap, "pdf")
    codon_circos(cmap, "pdf")
